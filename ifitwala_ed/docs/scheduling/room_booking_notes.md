...
**Rooms are operational truth only through Location Booking (room fact table).**

* **Location Booking** (new) = **room operational truth** (free/busy + analytics)
...
* Domain docs materialize into Location Booking for room truth.
...
**Location Booking makes every module ask the same question against the same truth.**
...
* **Location Booking**: room free/busy
...
## 3. Location Booking — required behavior

### 3.1 One row means one room is busy

A single Location Booking row means:

> location L is occupied for `[from_datetime, to_datetime)`

### 3.2 What materializes into Location Booking

Location Booking receives rows from:
...
Location Booking must support edits safely.
...
Location Booking is updated only by write-triggered materialization.

### 5.1 Meeting

* on insert/update: upsert Location Booking for meeting location
...
Free-room checks query **Location Booking only**.
...
This helper queries Location Booking only.

### 6.3 Room utilization analytics

Utilization queries Location Booking only.
...
Location Booking is operationally sensitive.
...
1. verify Location Booking rows exist for the time range
