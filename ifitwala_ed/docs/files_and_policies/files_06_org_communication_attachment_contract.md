...
8. Archive/detail surfaces must prefer `thumbnail_url` for inline card previews. For images, that means the `thumb` derivative. For PDFs, that means the smaller `pdf_card` first-page image while `preview_url` keeps the richer `pdf_page_1` preview action. If the thumbnail/card preview is missing but `preview_status` is `ready`, the surface may use `preview_url` as an inline fallback because that route resolves a governed preview derivative rather than the original file. If neither is available, the surface must fall back to an action-led card.
...
11. Open and preview routes must not require a secondary `File` row lookup when a safe Drive grant URL is already resolved. If no safe public/external/Drive target exists, the route must fail closed instead of streaming local private bytes. Thumbnail routes for Org Communication must fail closed when no safe `thumb` derivative grant exists and must never stream or redirect to the original file.
