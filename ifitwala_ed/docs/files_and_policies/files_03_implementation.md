...
- the legacy profile-image recovery path is patch-driven, not a permanent runtime fallback; the repair patches walk Employee, Student, Guardian, and admissions applicant/guardian profile-image rows, rebuild or resync missing governed profile images through the public Drive upload seam, and materialize refreshed compact/card/richer preview variants for current governed profile images during migrate so avatar surfaces stop falling back to originals after legacy repair; linked Employee `User.user_image` rows are normalized through `ifitwala_ed.patches.backfill_employee_user_images`, public website Employee image rows without linked users are normalized through `ifitwala_ed.patches.backfill_public_employee_profile_images`, and Student rows are normalized through `ifitwala_ed.patches.backfill_student_profile_images`
...
- public website people surfaces request missing Drive-managed Employee profile-image derivatives when a current governed profile image exists, then continue to render initials until Drive marks a public derivative ready and invalidates the cached people payload
