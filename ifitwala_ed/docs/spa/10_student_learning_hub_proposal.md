...
| Home payload | Student home now uses one aggregated Hub payload for identity, top orientation, server-curated board lanes, and dated timeline items | Upgrade to a `Today` cockpit with a top orientation strip, a compact `My Work Board`, and a dated learning timeline | Needs a future weekly lesson spine beyond today's schedule API and later focus/resume layers |
