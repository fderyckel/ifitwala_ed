...
| Home payload | Student home now uses one aggregated Hub payload for identity, top orientation, server-curated board lanes, dated timeline items, and server-owned course-readiness state for fallback course entry | Upgrade to a `Today` cockpit with a top orientation strip, a compact `My Work Board`, and a dated learning timeline | Needs a future weekly lesson spine beyond today's schedule API and later focus/resume layers |
| Planning board | A first compact board now exists with server-curated `Now / Soon / Later / Done` lanes derived from `Task Delivery` + `Task Outcome` truth | Add a compact kanban-style planning layer with student-managed `Now` and system-curated `Soon/Later/Done` support | Needs a student-side planning state model before manual lane moves, subtasks, or estimates can persist |
