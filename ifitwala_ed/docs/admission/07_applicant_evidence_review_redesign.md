...
- `Admission Officer`, `Admission Manager`, `Academic Admin`, and `System Manager` review evidence from applicant context only:
  - Desk `Student Applicant.documents_summary` quick actions
  - Admissions Cockpit applicant workspace overlay
- admissions staff have dual entry points but one workflow contract:
  - both Desk and workspace read the same server-built requirement/submission readiness payload
  - both surfaces act on requirement overrides at the parent level and submission review at the item level
- cockpit document blockers and review CTAs open the applicant workspace with requirement or submission anchors; they do not route staff into raw `Applicant Document` Desk records
...
- assigned interviewers, including non-admissions staff, use the same `AdmissionsWorkspaceOverlay` from the interview event route or Focus feedback item so interview prep, applicant brief, evidence, and in-panel feedback stay in one overlay
...
- admissions cockpit document blockers deep-linking to raw `Applicant Document` Desk records
...
| SPA / UI surfaces | applicant requirement cards, admissions workspace overlay with requirement/submission anchors, Desk applicant evidence summary quick actions, Focus for non-admissions reviewers only | Implemented |
| Reports / dashboards / briefings | Desk applicant summary and admissions cockpit both consume the same requirement/submission readiness model; cockpit blockers open workspace anchors instead of raw parent records | Implemented |
