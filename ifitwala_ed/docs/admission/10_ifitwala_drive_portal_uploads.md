...
* applicant-facing SPA responses now return one top-level governed `attachment` row containing server-owned admissions `open_url` / `preview_url` / `thumbnail_url` actions plus `drive_file_id` and `canonical_ref`
