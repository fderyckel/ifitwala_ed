...
ApplicantProfilePayload {
  ...
  applicant_image: string   // server-owned admissions open URL or ""
  options: {
    genders: string[]
...
* Profile image upload must route through the admissions Drive-governed upload contract:
  ...
  * `slot = profile_image`
  * `upload_source = SPA`
  * upload response returns a server-owned admissions `image_url` plus the compatibility `File` id, `drive_file_id`, and `canonical_ref`
...
* Uploaded profile image remains private; `Student Applicant.applicant_image` keeps the compatibility `File.file_url`, while portal DTOs expose only the server-owned admissions open URL.
* Guardian photo upload remains private, is normalized with the same rules, stores the compatibility `File.file_url` on `Student Applicant Guardian.guardian_image`, and returns only the server-owned admissions open URL to the SPA.
