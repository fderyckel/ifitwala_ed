...
* Profile image upload must route through the admissions Drive-governed upload contract:
  ...
  * `slot = profile_image`
  * `upload_source = SPA`
  * upload response returns a server-owned admissions thumbnail `image_url`, a separate admissions `open_url`, plus the compatibility `File` id, `drive_file_id`, and `canonical_ref`
...
* Uploaded profile image remains private; `Student Applicant.applicant_image` keeps the compatibility `File.file_url`, while portal DTOs expose a thumbnail-only display URL plus a separate server-owned admissions open URL.
* Guardian photo upload remains private, is normalized with the same rules, stores the compatibility `File.file_url` on `Student Applicant Guardian.guardian_image`, and returns `guardian_image` (thumbnail display URL) plus `guardian_image_open_url` (server-owned admissions open URL) to the SPA.
