...
- those workspaces render inline image previews from `thumbnail_url`, render inline PDF first-page cards from `thumbnail_url` when the card preview is ready, fall back to the governed `preview_url` inline when the card preview is absent or the thumbnail route cannot be delivered, and keep `preview_url` as the richer PDF preview action
...
- current supporting-material upload classification uses purpose `learning_resource`; file-backed materials bind through the canonical `supporting_material` Drive binding role, and legacy `general_reference` rows are remediated through the one-shot patch `ifitwala_ed.patches.backfill_supporting_material_binding_roles`
