<!-- ifitwala_ed/docs/website/05_school_slug_vs_page_route.md -->
# School Slug vs Website Page Route (Canonical)

**Audience:** Website admins, implementers, and content editors
**Scope:** Builder-lite public website routing
**Status (April 11, 2026):** Implemented and enforced by route rules, public-home rendering, directory rendering, and `School Website Page` validation

---

## 0. Root route ownership (`/`)

The public root route `/` is the **school-first public homepage**.

* `/` renders a public landing page for the top-most active organization scope.
* Legacy aliases `/home`, `/index`, and `/index.html` resolve to the same public home behavior.
* `Organization.default_website_school`, when valid and published, is used to spotlight the featured school on `/`.
* If no featured/default school is available, the home page falls back to the first published school inside the same organization scope for spotlight content.
* `/schools` is the public school directory/discovery page.
* School websites are scoped under `/schools/...`.
* Desk entry `/desk` remains framework-owned and outside website page resolution.

This keeps the out-of-the-box public experience school-first while making login an explicit utility action instead of the public front door.

---

## 1. Two different concepts (do not mix them)

### 1.1 `School.website_slug` (identity)

**What it is**

* The unique URL identity for a school
* Used to resolve which school owns `/schools/{school_slug}/...`

**Where it lives**

* DocType: `School`
* Field: `website_slug`

**Example**

```
School: Ifitwala Secondary School
website_slug: iss
```

This means the school home is `/schools/iss`.

---

### 1.2 `School Website Page.route` (page path input)

**What it is**

* The page path input typed by website managers
* Stored exactly as entered (no slug prefixing)

**Rules (enforced)**

* `/` is only for the school home page
* All other pages must be entered without a leading `/`
  * Examples: `about`, `admissions`, `about/team`
* No trailing `/`
* No empty segments (`//`)
* Do not include the school slug

**Where it lives**

* DocType: `School Website Page`
* Field: `route`

---

### 1.3 `School Website Page.full_route` (canonical)

**What it is**

* The canonical route used for matching and rendering
* Computed by the system from `School.website_slug` + `route`
* Read-only and system-owned

**Where it lives**

* DocType: `School Website Page`
* Field: `full_route`

---

## 2. The hard rule (enforced)

Route input is always school-relative, and canonical full routes are always under `/schools/{school_slug}`.

If `School.website_slug = iss`:

| User input (`route`) | Stored `route` | Stored `full_route` |
| --- | --- | --- |
| `/` | `/` | `/schools/iss` |
| `admissions` | `admissions` | `/schools/iss/admissions` |
| `about/team` | `about/team` | `/schools/iss/about/team` |

**Invalid inputs**

* `/admissions` (leading `/` not allowed for non-root)
* `iss/admissions` (school slug is never part of the input)
* `admissions/` (trailing `/` not allowed)

---

## 3. Root page behavior

If the route is `/`, the system creates the school home page:

```
/schools/{school_slug}
```

Only one root page is allowed per school.

---

## 4. Routing resolution model

For school website requests:

1. `/schools/{school_slug}/...` resolves the school by `website_slug`
2. Full path is matched against `School Website Page.full_route`
3. The page is rendered for that school

For school directory requests:

1. `/schools` renders the public school directory
2. Directory lists published schools with valid `website_slug`

For public home requests:

1. `/` (and aliases) resolve the top-most active organization
2. Render a public landing page owned by trusted app code
3. Prefer `Organization.default_website_school` when it is published and has a valid `website_slug` for featured-school content
4. Otherwise choose the first published school under that organization scope (`School.lft asc`) for featured-school content
5. Login remains a separate `/login` utility route

For admissions/public form requests:

1. `/apply/inquiry` is the native Web Form route.
2. Legacy `/inquiry` redirects to that canonical `/apply/inquiry` route.
3. Admissions applicant SPA is namespaced under `/admissions/*` and does not own `/apply/*`.

For authenticated portal requests:

1. Canonical authenticated portal namespace is `/hub/*`.

---

## 5. Summary (one-line rule)

> **`/` is the public homepage; `/schools` is the public school directory; school pages live under `/schools/{school_slug}/...`; `Organization.default_website_school` now controls featured-school spotlighting, not root redirection; `route` is user input; `full_route` is canonical.**
