...
- Reviewed in the staff SPA through `ifitwala_ed.api.term_reporting.get_review_surface`, which returns the selected Reporting Cycle, readiness checks, bounded result rows, and component breakdowns in one scoped payload.
...
- **Read API**:
  ...
  - `api/term_reporting.py::get_review_surface()` returns the bounded staff review payload for cycle selection, readiness checks, Course Term Result rows, and component explainability
  - `api/term_reporting.py::queue_review_action()` queues approved recalculation/report-generation jobs; it does not run heavy reporting generation in the request path
- **Performance indexes**:
