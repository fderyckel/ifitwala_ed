...
summary: "Link an employee with user identity, school scope, and computed teaching context so schedule, class, and visibility flows can treat them as an instructor."
...
- It builds the read-only `Instructor Log` snapshot from current Student Group assignments.
...
2. On save, the controller syncs:
   ...
   - `instructor_name`
   - `instructor_image`
3. The controller rebuilds `instructor_log` from active `Student Group Instructor` links.
...
- `before_save()` always rebuilds the computed `instructor_log`.
...
- `sync_instructor_logs(...)` is the bounded rebuild helper used when Student Group instructor membership changes.

### Operational Queries and Public Methods

- `get_instructor_log(instructor)`
  returns a fresh rebuilt instructor log payload
...
- `instructor_log` is computed from Student Group assignment data and should stay derived, not hand-edited.
