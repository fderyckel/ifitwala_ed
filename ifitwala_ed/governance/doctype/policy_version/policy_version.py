...
class PolicyVersion(Document):
	...
	def _validate_parent_policy(self):
		...
		active = frappe.db.get_value(
			"Institutional Policy", self.institutional_policy, "is_active"
		)
		if not active:
	...
	def _enforce_ack_lock(self, before):
		has_ack = frappe.db.exists(
			"Policy Acknowledgement", {"policy_version": self.name}
		)
		if not has_ack:
			return
		...
		if not is_system_manager() or not override_reason:
			frappe.throw(
				_("Policy Version is immutable after acknowledgements exist. "
		...
		self.add_comment(
			"Comment",
			text=_(
				"System Manager override on Policy Version by {0} at {1}. Reason: {2}."
			).format(frappe.bold(frappe.session.user), now_datetime(), override_reason),
		)

	def _validate_active_state(self):
		if not self.is_active:
			return
		active_parent = frappe.db.get_value(
			"Institutional Policy", self.institutional_policy, "is_active"
		)
