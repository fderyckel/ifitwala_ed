...
class Meeting(Document):
    ...
    def validate_employee_conflicts(self) -> None:
        ...
        allow_double_booking = bool(getattr(self, "ignore_conflict", getattr(self, "ignore_conflicts", 0)))
    ...
    def sync_employee_bookings(self) -> None:
        ...
        school = getattr(self, "school", None) if hasattr(self, "school") else None
        academic_year = getattr(self, "academic_year", None) if hasattr(self, "academic_year") else None
    ...
    def _user_is_participant(self, user: str) -> bool:
        return any(row.participant == user for row in (self.participants or []) if row.participant)
