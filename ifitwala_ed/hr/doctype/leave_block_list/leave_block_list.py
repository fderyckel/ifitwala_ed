...
def get_applicable_block_dates(from_date, to_date, employee=None, organization=None, all_lists=False, leave_type=None):
    return frappe.db.get_all(
        "Leave Block List Date",
        filters={
            "parent": ["IN", get_applicable_block_lists(employee, organization, all_lists, leave_type)],
...
def get_applicable_block_lists(employee=None, organization=None, all_lists=False, leave_type=None):
    block_lists = []
    employee_school = None
    ...
    if not organization and employee:
        ...
        employee_school = frappe.db.get_value("Employee", employee, "school")

    if organization:
        # global
        conditions = {"applies_to_all_departments": 1, "organization": organization}
        if employee_school:
            conditions["school"] = ["in", [employee_school, "", None]]
        if leave_type:
