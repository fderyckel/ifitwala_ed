...
class LeavePolicyAssignment(Document):
    ...
    def set_scope_fields(self):
        if not self.employee:
            return
        self.organization, self.school = frappe.db.get_value("Employee", self.employee, ["organization", "school"])
    ...
    def create_leave_allocation(self, annual_allocation, leave_details, date_of_joining):
        ...
        earned_leave_schedule = (
            self.get_earned_leave_schedule(annual_allocation, leave_details, date_of_joining, new_leaves_allocated)
    ...
    def get_new_leaves(self, annual_allocation, leave_details, date_of_joining):
        ...
        # if earned leave is being allcated after the effective period, then let them be calculated pro-rata
            ...
            new_leaves_allocated = self.get_leaves_for_passed_period(annual_allocation, leave_details, date_of_joining)
        else:
    ...
    def calculate_leaves_for_passed_period(
        ...
        if getdate(self.effective_from) <= date_of_joining <= period_end_date:
            ...
            start_date, end_date = get_sub_period_start_and_end(date_of_joining, leave_details.earned_leave_frequency)
...
def calculate_periods_passed(current_date, from_date, periods_per_year, months_per_period, consider_current_period):
    periods_passed = 0
