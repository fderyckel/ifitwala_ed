...
from ifitwala_ed.hr import leave_api
from ifitwala_ed.hr.doctype.leave_block_list.leave_block_list import get_applicable_block_dates
from ifitwala_ed.hr.doctype.leave_ledger_entry.leave_ledger_entry import create_leave_ledger_entry
from ifitwala_ed.hr.leave_api import PWANotificationsMixin, get_employee_email
...
class LeaveApplication(Document, PWANotificationsMixin):
    ...
    def validate(self):
        ...
        self.set_organization_and_school()
        self.resolve_leave_approver()
    ...
    def set_organization_and_school(self):
        if not self.employee:
            return
        employee_org, employee_school = frappe.db.get_value("Employee", self.employee, ["organization", "school"])
        self.organization = employee_org
        if self.meta.has_field("school"):
            self.school = employee_school

    def resolve_leave_approver(self):
        if not self.employee:
            return
        if not self.leave_approver:
            self.leave_approver = frappe.db.get_value("Employee", self.employee, "leave_approver")

        if (
            frappe.db.get_single_value("HR Settings", "leave_approver_mandatory_in_leave_application")
            and not self.leave_approver
        ):
            frappe.throw(_("Leave Approver is mandatory for Leave Application."))
    ...
    def validate_status_transition(self):
        ...
        prev_status = prev.status if prev else "Open"
        ...
        if next_status in {"Approved", "Rejected"}:
            if not (is_hr_override or (self.leave_approver and frappe.session.user == self.leave_approver)):
                frappe.throw(_("Only assigned leave approver or HR override roles can approve/reject leaves."))

        if next_status == "Approved":
            prevent_self = frappe.db.get_single_value("HR Settings", "prevent_self_leave_approval")
            if prevent_self and employee_user and employee_user == frappe.session.user and not is_hr_override:
    ...
    def on_submit(self):
        ...
        if to_date < getdate() and can_expire:
            args = frappe._dict(leaves=self.total_leave_days, from_date=to_date, to_date=to_date, is_carry_forward=0)
    ...
    def publish_update(self):
        ...
        leave_api.refetch_resource("ifitwala_ed:my_leaves", employee_user)
        leave_api.refetch_resource("ifitwala_ed:team_leaves")
    ...
    def update_attendance(self):
        ...
        for dt in daterange(getdate(self.from_date), getdate(self.to_date)):
            ...
            if date in holiday_dates:
                if attendance_name:
                    attendance = frappe.get_doc("Employee Attendance", attendance_name)
            ...
            doc.db_set(
                {
                    ...
                    "source_transaction_type": "Leave Application",
                    "source_transaction_name": self.name,
                }
            )
        else:
            doc = frappe.new_doc("Employee Attendance")
            doc.employee = self.employee
            doc.employee_full_name = self.employee_full_name
            doc.attendance_date = date
            doc.organization = self.organization
            doc.school = self.get("school")
            doc.department = self.department
            doc.leave_type = self.leave_type
            doc.leave_application = self.name
            doc.status = status
            doc.half_day_status = "Present" if status == "Half Day" else None
            doc.attendance_method = "Manual"
            doc.source_transaction_type = "Leave Application"
            doc.source_transaction_name = self.name
    ...
    def cancel_attendance(self):
        if self.docstatus == 2:
            attendance = frappe.db.sql(
                """select name from `tabEmployee Attendance` where employee = %s\
				and (attendance_date between %s and %s) and docstatus < 2 and status in ('On Leave', 'Half Day')""",
                (self.employee, self.from_date, self.to_date),
                as_dict=1,
            )
            for name in attendance:
                doc = frappe.get_doc("Employee Attendance", name.name)
                doc.cancel()
    ...
    def validate_block_days(self):
        block_dates = get_applicable_block_dates(
            self.from_date, self.to_date, self.employee, self.organization, leave_type=self.leave_type
        )
    ...
    def validate_optional_leave(self):
        leave_period = get_leave_period(self.from_date, self.to_date, self.organization)
        if not leave_period:
        ...
        while day <= getdate(self.to_date):
            in_staff_calendar = frappe.db.exists(
                {
                    "doctype": "Staff Calendar Holidays",
    ...
    def create_separate_ledger_entries(self, alloc_on_from_date, alloc_on_to_date, submit, lwp):
        ...
        if leaves_in_first_alloc:
            args.update(dict(from_date=self.from_date, to_date=first_alloc_end, leaves=leaves_in_first_alloc * -1))
            create_leave_ledger_entry(self, args, submit)

        if leaves_in_second_alloc:
            args.update(dict(from_date=second_alloc_start, to_date=self.to_date, leaves=leaves_in_second_alloc * -1))
            create_leave_ledger_entry(self, args, submit)
    ...
    def validate_for_self_approval(self):
        self_leave_approval_not_allowed = frappe.db.get_single_value("HR Settings", "prevent_self_leave_approval")
        employee_user = frappe.db.get_value("Employee", self.employee, "user_id")
        roles = set(frappe.get_roles())
        is_hr_override = bool(roles & HR_OVERRIDE_ROLES)
        if (
...
def get_leave_balance_on(
    ...
    end_date = allocation.to_date if (allocation and cint(consider_all_leaves_in_the_allocation_period)) else date
    cf_expiry = get_allocation_expiry_for_cf_leaves(employee, leave_type, to_date, allocation.from_date)

    leaves_taken = get_leaves_for_period(employee, leave_type, allocation.from_date, end_date)
    manually_expired_leaves = get_manually_expired_leaves(employee, leave_type, allocation.from_date, end_date)

    remaining_leaves = get_remaining_leaves(allocation, leaves_taken, date, cf_expiry, manually_expired_leaves)
...
def get_leave_allocation_records(employee, date, leave_type=None):
    ...
    query = (
        ...
        .where(
            ...
            & (Ledger.docstatus == 1)
            & ((Ledger.transaction_type == "Leave Allocation") | (Ledger.transaction_type == "Leave Adjustment"))
            & (Ledger.employee == employee)
            & (Ledger.is_expired == 0)
            & (Ledger.is_lwp == 0)
            & (
...
def get_manually_expired_leaves(employee: str, leave_type: str, from_date: datetime.date, end_date: datetime.date):
    ledger = frappe.qb.DocType("Leave Ledger Entry")
...
def get_new_and_cf_leaves_taken(allocation: dict, cf_expiry: str) -> tuple[float, float]:
    ...
    cf_leaves_taken = get_leaves_for_period(allocation.employee, allocation.leave_type, allocation.from_date, cf_expiry)
    ...
    for leave_entry in leave_entries:
        ...
        if inclusive_period and leave_entry.transaction_type == "Leave Encashment":
            ...
            if leave_entry.leaves % 1:
                half_day = 1
                half_day_date = frappe.db.get_value("Leave Application", leave_entry.transaction_name, "half_day_date")
...
def get_events(start, end, filters=None):
    ...
    employee = frappe.db.get_value(
        "Employee", filters={"user_id": frappe.session.user}, fieldname=["name", "organization"], as_dict=True
    )
    ...
    if employee:
        ...
        employee = ""
        organization = frappe.db.get_single_value("Global Defaults", "default_organization")
...
def add_department_leaves(events, start, end, employee, organization):
    if department := frappe.db.get_value("Employee", employee, "department"):
        department_employees = frappe.get_list(
            "Employee", filters={"department": department, "organization": organization}, pluck="name"
        )
...
def add_leaves(events, start, end, filters=None):
    ...
    for d in leave_applications:
        d["title"] = f"{d['employee_full_name']} ({d['leave_type']})"
        d["allDay"] = 1
        d["doctype"] = "Leave Application"
        del d["employee_full_name"]
        del d["leave_type"]
        if d not in events:
            events.append(d)
        ...
        events.append(
            {
                ...
                "name": f"holiday-{holiday.holiday_date}",
                "allDay": 1,
            }
        )
...
def get_mandatory_approval(doctype):
    if doctype == "Leave Application":
        return frappe.db.get_single_value("HR Settings", "leave_approver_mandatory_in_leave_application")
    return 0
...
def get_approved_leaves_for_period(employee, leave_type, from_date, to_date):
    ...
    for leave_app in leave_applications:
        if leave_app.from_date >= getdate(from_date) and leave_app.to_date <= getdate(to_date):
            ...
            leave_days += get_number_of_leave_days(employee, leave_type, leave_app.from_date, leave_app.to_date)
