...
from ifitwala_ed.hr.leave_api import get_current_employee_info
...
from ifitwala_ed.hr.leave_api import PWANotificationsMixin
from ifitwala_ed.hr.leave_api import get_employee_email
from ifitwala_ed.hr.utils import get_holiday_dates_between_range
...
class LeaveApplication(Document, PWANotificationsMixin):
	...
	def set_organization_and_school(self):
		if not self.employee:
			return
		employee_org, employee_school = frappe.db.get_value(
	...
	def validate_status_transition(self):
		...
		if next_status not in allowed.get(prev_status, set()):
			frappe.throw(
				_("Invalid status transition from {0} to {1}.").format(prev_status, next_status)
			)
