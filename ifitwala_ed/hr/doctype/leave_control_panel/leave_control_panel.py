...
from ifitwala_ed.hr.utils import validate_bulk_tool_fields


def _get_default_organization():
    return frappe.db.get_single_value("Global Defaults", "default_organization")
...
class LeaveControlPanel(Document):
    ...
    def create_leave_allocations(self, employees: list) -> dict:
        ...
        for employee in employees:
            try:
                ...
                allocation.from_date = from_date or frappe.db.get_value("Employee", employee, "date_of_joining")
                ...
                allocation.insert()
                allocation.submit()
                success.append({"doc": get_link_to_form("Leave Allocation", allocation.name), "employee": employee})
            except Exception:
                frappe.db.rollback(save_point=savepoint)
                frappe.log_error(
                    title="Leave Allocation Bulk Failure",
                    message=f"Leave Allocation failed for employee {employee}",
                )
    ...
    def create_leave_policy_assignments(self, employees: list) -> dict:
        ...
        for employee in employees:
            try:
                ...
                assignment.effective_from = from_date or frappe.db.get_value("Employee", employee, "date_of_joining")
                ...
                frappe.db.rollback(save_point=savepoint)
                frappe.log_error(
                    title="Leave Policy Assignment Bulk Failure",
                    message=f"Leave Policy Assignment failed for employee {employee}",
                )
                failure.append(employee)
    ...
    def get_employees(self, advanced_filters: list) -> list:
        ...
        if to_date and (from_date or self.dates_based_on == "Joining Date"):
            if all_employees := frappe.get_list(
                ...
                fields=["name", "employee_full_name", "organization", "department", "date_of_joining"],
            ):
    ...
    def get_employees_without_allocations(self, all_employees: list, from_date: str, to_date: str) -> list:
        ...
        if self.allocate_based_on_leave_policy and self.leave_policy:
            leave_types = frappe.get_all("Leave Policy Detail", {"parent": self.leave_policy}, pluck="leave_type")
    ...
    def get_latest_leave_period(self):
        return frappe.db.get_value(
            ...
            {
                "is_active": 1,
                "organization": self.organization or _get_default_organization(),
            },
            "name",
