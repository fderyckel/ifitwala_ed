...
class ApplicantDocument(Document):
	...
	def get_file_routing_context(self, file_doc):
		doc_type_code = frappe.db.get_value(
			"Applicant Document Type", self.document_type, "code"
	...
	def _validate_unique_document_type(self):
		...
		if exists:
			frappe.throw(
				_("Only one Applicant Document per document type is allowed per Applicant.")
			)
