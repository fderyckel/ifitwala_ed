# ifitwala_ed/admission/api/review.py
...
def review_applicant_document_submission_impl(
    *,
    ...
    with cache.lock(_lock_key(user, lock_target), timeout=10):
        ...
        (invalidate_cache or invalidate_admissions_cockpit_cache)()
        if cache_key:
            cache.set_value(cache_key, response, expires_in_sec=60 * 10)
        return response


def set_document_requirement_override_impl(
    *,
    student_applicant: str | None = None,
