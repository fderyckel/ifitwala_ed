# ifitwala_ed/admission/api/test_recommendation_intake.py
...
class TestRecommendationIntake(FrappeTestCase):
    ...
    def test_staff_review_payload_handles_select_field_options(self):
        template = frappe.get_doc(
            {
                "doctype": "Recommendation Template",
                "template_name": f"Select Recommendation {frappe.generate_hash(length=5)}",
                "is_active": 1,
                "organization": self.organization,
                "school": self.school,
                "target_document_type": self.document_type,
                ...
                "template_fields": [
                    {
                        ...
                        "field_type": "Select",
                        "is_required": 1,
                        "options_json": "Strongly Recommend\nRecommend\nDo Not Recommend",
                    }
                ],
            }
        ...
        created = create_recommendation_request(
            student_applicant=self.applicant.name,
            recommendation_template=template.name,
            recommender_name="Ms Select",
            recommender_email=f"select-{frappe.generate_hash(length=6)}@example.com",
            recommender_relationship="Teacher",
            send_email=0,
            client_request_id=f"select-create-{frappe.generate_hash(length=6)}",
        )
        ...
        submit_recommendation(
            token=token,
            answers={"overall_recommendation": "Strongly Recommend"},
            attestation_confirmed=1,
            client_request_id=f"select-submit-{frappe.generate_hash(length=6)}",
        )
        ...
        self.assertEqual(len(answers), 1)
        self.assertEqual((answers[0] or {}).get("field_type"), "Select")
        self.assertEqual((answers[0] or {}).get("display_value"), "Strongly Recommend")
        self.assertEqual((answers[0] or {}).get("likert_columns"), [])
        self.assertEqual((answers[0] or {}).get("likert_rows"), [])
