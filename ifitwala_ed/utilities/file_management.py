...
def ensure_folder(path: str) -> str:
    ...
    if not frappe.db.exists("File", "Home"):
        frappe.get_doc({"doctype": "File", "name": "Home", "file_name": "Home", "is_folder": 1, "folder": ""}).insert(
    ...
    for part in parts[1:]:
        ...
        if not frappe.db.exists("File", {"file_name": part, "is_folder": 1, "folder": current}):
            frappe.get_doc({"doctype": "File", "file_name": part, "is_folder": 1, "folder": current}).insert(
                ignore_permissions=True, ignore_if_duplicate=True
            )
        current = next_folder
