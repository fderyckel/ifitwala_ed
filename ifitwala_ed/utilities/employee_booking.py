...
def assert_employee_free(
	...
	*,
	...
	Typical usage in Meeting.validate():
	    ...
	    for p in self.participants:
	        if p.employee:
	            assert_employee_free(
	                ...
	                allow_double_booking=self.ignore_conflicts,
	            )
	"""
	...
	for c in conflicts:
		lines.append(
			_("{booking_type} ({source_doctype} {source_name}) from {from_dt} to {to_dt}").format(
				booking_type=c.booking_type,
				source_doctype=c.source_doctype,
				source_name=c.source_name,
				from_dt=frappe.format(c.start, "Datetime"),
				to_dt=frappe.format(c.end, "Datetime"),
			)
		)

	msg = "<br>".join(lines)
	frappe.throw(
		_("Employee {0} is already booked in this time window:<br>{1}").format(employee, msg),
...
def upsert_employee_booking(
	*,
	...
	"""
	Create or update a single Employee Booking row for a given
	(employee, source_doctype, source_name) triple.
	...
	if end_dt <= start_dt:
		...
		delete_employee_bookings_for_source(source_doctype, source_name, employee=employee)
		return ""

	# Look up existing booking name in the cheapest way possible.
	...
	if existing_name:
		# Update in place; avoid loading full doc if we can.
		frappe.db.set_value(
			"Employee Booking",
			existing_name,
			{
