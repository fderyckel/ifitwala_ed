...
def _validate_classification_payload(classification: Dict[str, Any]) -> None:
    ...
    if missing:
        frappe.throw(_("Missing mandatory file classification fields: {0}").format(", ".join(missing)))
...
def _resolve_retention_until(classification: Dict[str, Any]):
    if not frappe.db.exists("DocType", "File Retention Policy"):
        frappe.logger().warning("[file_dispatcher] File Retention Policy doctype missing; retention_until left empty.")
        return None
    ...
    if retention_days is None:
        frappe.throw(
            _("No retention policy defined for data_class '{0}' and purpose '{1}'").format(
                classification["data_class"], classification["purpose"]
            )
        )
