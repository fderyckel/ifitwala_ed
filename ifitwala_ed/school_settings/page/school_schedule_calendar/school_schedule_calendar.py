...
def get_schedule_events(school=None, academic_year=None):
	...
	for sched in schedules:
		...
		while date_cursor <= end_date:
			...
			# 2. If it's a holiday, don't add blocks, but let rotation increment if include_holidays_in_rotation
			if is_holiday:
				date_cursor += timedelta(days=1)
				rotation_index += 1 if sched.include_holidays_in_rotation else 0
				continue
