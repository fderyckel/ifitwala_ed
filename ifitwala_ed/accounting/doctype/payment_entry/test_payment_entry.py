...
class TestPaymentEntry(FrappeTestCase):
    ...
    def test_payment_against_sales_order_usd_to_inr(self):
        ...
        expected_gle = dict(
            (d[0], d) for d in [["_Test Receivable USD - _TC", 0, 5500, pe.name], [pe.paid_to, 5500.0, 0, None]]
        )
    ...
    def test_multi_currency_payment_entry_with_taxes(self):
        payment_entry = create_payment_entry(party="_Test Supplier USD", paid_to="_Test Payable USD - _TC", save=True)
    ...
    def test_details_update_on_reference_table(self):
        ...
        ref_details = get_reference_details(so.doctype, so.name, pe.paid_from_account_currency, "Customer", so.customer)
        expected_response = {
    ...
    def test_opening_flag_for_advance_as_liability(self):
        ...
        gl_with_opening_set = frappe.db.get_all("GL Entry", filters={"voucher_no": adv.name, "is_opening": "Yes"})
        ...
        payment.save()
        payment.submit()
        gl_with_opening_set = frappe.db.get_all("GL Entry", filters={"voucher_no": payment.name, "is_opening": "Yes"})
...
def create_payment_entry(**args):
    ...
    get_party_details(payment_entry.company, payment_entry.party_type, payment_entry.party, payment_entry.posting_date)
...
def create_payment_terms_template_with_discount(name=None, discount_type=None, discount=None, template_name=None):
    """
