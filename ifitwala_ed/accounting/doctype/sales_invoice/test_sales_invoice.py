...
class TestSalesInvoice(FrappeTestCase):
    ...
    def validate_pos_gl_entry(self, si, pos, cash_amount, validate_without_change_gle=False):
        ...
        self.assertTrue(sle)
        self.assertEqual([sle.item_code, sle.warehouse, sle.actual_qty], ["_Test FG Item", "Stores - TCP1", -1.0])
    ...
    def test_sales_invoice_gl_entry_with_perpetual_inventory_no_item_code(self):
        ...
        expected_values = dict((d[0], d) for d in [["Debtors - TCP1", 100.0, 0.0], ["Sales - TCP1", 0.0, 100.0]])
        for _i, gle in enumerate(gl_entries):
            self.assertEqual(expected_values[gle.account][0], gle.account)
            self.assertEqual(expected_values[gle.account][1], gle.debit)
            self.assertEqual(expected_values[gle.account][2], gle.credit)
    ...
    def test_credit_note(self):
        ...
        outstanding_amount = get_outstanding_amount(si.doctype, si.name, "Debtors - _TC", si.customer, "Customer")
    ...
    def test_sle_for_target_warehouse(self):
        ...
        sles = frappe.get_all("Stock Ledger Entry", filters={"voucher_no": si.name}, fields=["name", "actual_qty"])
    ...
    def test_internal_transfer_gl_entry(self):
        ...
        check_gl_entries(self, target_doc.name, pi_gl_entries, add_days(nowdate(), -1), voucher_type="Purchase Invoice")
    ...
    def test_sales_invoice_against_supplier_usd_with_dimensions(self):
        ...
        # create a sales invoice
        si = create_sales_invoice(customer=customer, parent_cost_center="_Test Cost Center - _TC", do_not_submit=True)
    ...
    def test_sales_invoice_with_disabled_account(self):
        try:
            ...
            self.assertTrue("Cannot create accounting entries against disabled accounts" in str(err.exception))
    ...
    def test_pos_returns_without_update_outstanding_for_self(self):
        ...
        res = (
            ...
            .distinct()
            .where(gle.is_cancelled.eq(0) & gle.voucher_no.eq(pos_return.name) & gle.against_voucher.notnull())
            .run(as_list=1)
        )
...
def setup_accounts():
    ...
    frappe.db.set_value("Company", "_Test Company with perpetual inventory", "unrealized_profit_loss_account", account)
