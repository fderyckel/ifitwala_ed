// ifitwala_ed/school_site/doctype/course_website_profile/course_website_profile.js
...
function setupHeroImagePicker(frm) {
	frm.set_df_property(
		"hero_image",
		"description",
		__("Choose a governed organization media image for this course hero.")
	);
...
async function validatePreviewSeoRules(frm) {
	...
	if (ownsH1Count !== 1) {
		frappe.msgprint(
			__(
				"Preview requires exactly one enabled H1 owner block (for example Course Intro or Hero). Found {0}.",
				[String(ownsH1Count)]
			)
		);
		return false;
...
const WORKFLOW_METHOD =
	"ifitwala_ed.school_site.doctype.course_website_profile.course_website_profile.transition_workflow_state";
...
function updateDisallowedBlockWarning(frm, allowedTypes) {
	...
	frm.set_intro(
		__(
			"Some existing blocks are not allowed for Course Website Profile: {0}. Replace them before saving.",
			[disallowed.join(", ")]
		),
		"orange"
	);
...
async function syncAllowedBlockTypes(frm) {
	try {
		...
		frappe.msgprint(__("Unable to load allowed block types for Course Website Profile."));
		return [];
	}
}
...
function getSeoPayload(frm) {
	...
	return {
		...
		course: frm.doc.course || null,
		course_slug: frm.doc.course_slug || null,
...
frappe.ui.form.on("Course Website Profile", {
	async refresh(frm) {
		...
		frm.add_custom_button(__("Preview"), async () => {
			if (!frm.doc.school || !frm.doc.course_slug) {
				frappe.msgprint(__("Select a School and course slug to preview."));
				return;
			}
			...
			const previewUrl = `${getPreviewUrl(`/schools/${schoolSlug}/courses/${frm.doc.course_slug}`)}?preview=1`;
		...
		frm.add_custom_button(__("Add Block"), async () => {
			...
			if (!allowedTypes.length) {
				frappe.msgprint(__("No block types are configured for Course Website Profile."));
				return;
			}
	...
	async course(frm) {
		if (frm.doc.course && !frm.doc.school) {
			const school = await getFieldValue("Course", frm.doc.course, "school");
			if (school && !frm.doc.school) {
				await frm.set_value("school", school);
			}
		}
		refreshSeoAssistant(frm);
	},
