...
frappe.ui.form.on("Website Story", {
	refresh(frm) {
		...
		frm.add_custom_button(__("Preview"), async () => {
			...
			const previewUrl = `${getPreviewUrl(`/schools/${schoolSlug}/stories/${frm.doc.slug}`)}?preview=1`;
		...
		if (frm.dashboard && frm.dashboard.set_headline) {
			...
			if (frm.doc.workflow_state === "Published" && frm.doc.publish_at) {
				banners.push(__("This story is scheduled and will only go live after Publish At."));
			}
			if (frm.doc.workflow_state === "Published" && frm.doc.expire_at) {
				banners.push(__("This story will automatically return to draft after Expire At."));
			}
			if (frm.doc.school) {
				frappe.db
