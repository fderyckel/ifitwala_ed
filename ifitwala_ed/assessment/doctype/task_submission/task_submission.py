...
def _ensure_indexes():
    ...
    if not _unique_index_exists(rows, ["task_outcome", "version"]):
        frappe.db.sql(f"ALTER TABLE `{table}` ADD UNIQUE INDEX `{unique_name}` (`task_outcome`, `version`)")
    ...
    if not _index_exists(rows, index_name):
        if _columns_exist(table, ["task_delivery", "student"]):
            frappe.db.sql(f"ALTER TABLE `{table}` ADD INDEX `{index_name}` (`task_delivery`, `student`)")
