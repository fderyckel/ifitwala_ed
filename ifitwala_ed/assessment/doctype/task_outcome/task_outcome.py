...
def _ensure_unique_index():
	...
	if existing:
		...
		if non_unique != 0:
			frappe.throw(
				"Task Outcome index exists but is not unique. "
				"Migration/installation is broken."
			)

	frappe.db.sql(
		f"ALTER TABLE `{table}` ADD UNIQUE INDEX `{index_name}` (`task_delivery`, `student`)"
	)
	...
	if not _unique_index_exists(rows, columns):
		frappe.throw(
			"Task Outcome UNIQUE(task_delivery, student) index missing after migrate. "
			"Migration/installation is broken."
		)
