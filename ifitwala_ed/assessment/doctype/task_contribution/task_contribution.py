...
from ifitwala_ed.assessment.task_outcome_service import apply_official_outcome_from_contributions, resolve_grade_symbol
...
def _ensure_indexes():
    ...
    if not _index_exists(rows, idx_outcome):
        frappe.db.sql(f"ALTER TABLE `{table}` ADD INDEX `{idx_outcome}` (`task_outcome`, `is_stale`, `modified`)")
    ...
    if not _index_exists(rows, idx_submission):
        frappe.db.sql(f"ALTER TABLE `{table}` ADD INDEX `{idx_submission}` (`task_submission`)")
    ...
    if not _index_exists(rows, idx_contributor):
        frappe.db.sql(f"ALTER TABLE `{table}` ADD INDEX `{idx_contributor}` (`contributor`, `modified`)")
