...
def _ensure_can_moderate_portfolio_item(
    *,
    ...
    if not allowed:
        frappe.throw(
            _("Not permitted to moderate this student's portfolio in the selected school."), frappe.PermissionError
        )
...
def _portfolio_item_dict(
    row: Dict[str, Any], tags_map: Dict[str, list[Dict[str, Any]]], evidence_map: Dict[str, Dict[str, Any]]
):
...
def list_reflection_entries(payload=None, **kwargs):
    ...
    scope = _resolve_actor_scope(
        requested_students=requested_students, school_filter=(data.get("school") or "").strip() or None
    )
    if scope.get("role") == "Guardian":
    ...
    doc = frappe.get_doc(
        {
            ...
            "allow_download": 1
            if int(data.get("allow_download") or settings.get("allow_external_download") or 0) == 1
            else 0,
