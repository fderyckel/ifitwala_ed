...
def _compute_block_1_pattern_counts(ctx: dict[str, Any]) -> dict[str, int]:
    ...
    return {row.student: int(row.get("block_1_absences") or 0) for row in rows if row.get("student")}
    ...
    return [
        {
            ...
            "attendance_rate": round(
                (int(row.get("present_sessions") or 0) / int(row.get("expected_sessions") or 1)) * 100,
                2,
            )
...
def _get_my_groups_payload(ctx: dict[str, Any], *, thresholds: dict[str, float]) -> dict[str, Any]:
    ...
    emerging_patterns.sort(
        key=lambda row: (row["absent_count"], row["late_count"], row["pattern_absent_block_1"]), reverse=True
    )
...
def _fetch_cached_result(
    *,
    ...
    cache_key = (
        "ifitwala_ed:attendance:analytics:"
        + hashlib.sha1(json.dumps(cache_payload, sort_keys=True, default=str).encode("utf-8")).hexdigest()
    )
