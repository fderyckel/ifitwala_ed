...
def _get_activity_booking_settings() -> dict:
    ...
    return {
        ...
        "default_paid_booking_portal_state": (doc.default_paid_booking_portal_state or PAID_PORTAL_STATE_CONFIRMED),
...
def _student_overlap_for_section(
    student: str, target_section: str, window_start, window_end, exclude_booking: str | None = None
) -> dict | None:
    """
...
def _assert_self_cancellation_allowed(doc, offering, actor: str) -> None:
    ...
    settings = _get_activity_booking_settings()
    mode = settings.get("default_guardian_student_cancellation_mode") or CANCELLATION_MODE_ALLOW_UNTIL_FIRST
...
def cancel_activity_booking(activity_booking: str, reason: str | None = None):
    ...
    return {
        ...
        "booking": _booking_payload(
            ...
            actor_type=actor,
            paid_portal_state=(
                _get_activity_booking_settings().get("default_paid_booking_portal_state") or PAID_PORTAL_STATE_CONFIRMED
            ),
        ),
...
def get_activity_portal_board(students=None, include_inactive: int = 0):
    ...
    for row in offering_rows:
        ...
        open_from = (
            get_datetime(row.get("activity_booking_open_from")) if row.get("activity_booking_open_from") else None
        )
        ...
        reserved_map = {
            (row.get("section_name") or "").strip(): cint(row.get("reserved_count") or 0) for row in reserved_rows
        }
    ...
    return {
        ...
        "settings": {
            ...
            "default_guardian_student_cancellation_mode": settings.get("default_guardian_student_cancellation_mode"),
...
@frappe.whitelist()
def get_activity_communications(
