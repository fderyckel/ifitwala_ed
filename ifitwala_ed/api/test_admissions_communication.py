...
class TestAdmissionsCommunicationCanonicalWriters(FrappeTestCase):
    ...
    def test_first_thread_creation_preserves_request_session(self):
        ...
        frappe.local.form_dict = frappe._dict({"context_name": "APP-0001"})
        observed = {}
        fake_cache = SimpleNamespace(lock=lambda *args, **kwargs: _NoopLock())

        def fake_create_thread(**kwargs):
            observed["user_during_insert"] = frappe.session.user
            return "COMM-0001"
        ...
        try:
            ...
            restored_user = frappe.session.user
            restored_sid = frappe.session.sid
            restored_csrf = frappe.session.data.csrf_token
            restored_form_context = frappe.local.form_dict.context_name
            frappe.local.session = original_session
            frappe.local.form_dict = original_form_dict

        self.assertEqual(thread_name, "COMM-0001")
        self.assertEqual(observed["user_during_insert"], "Administrator")
        self.assertEqual(restored_user, "applicant@example.com")
        self.assertEqual(restored_sid, "real-browser-session")
        self.assertEqual(restored_csrf, "csrf-token")
        self.assertEqual(restored_form_context, "APP-0001")
