...
def validate_posting_date(organization, posting_date):
	...
	if lock_until_date and getdate(posting_date) <= getdate(lock_until_date):
		frappe.throw(
			_("Posting date {0} is in a locked period for Organization {1}").format(
				posting_date, organization
			)
		)
	...
	if closed_period:
		frappe.throw(
			_("Posting date {0} falls in a closed Accounting Period").format(posting_date)
		)
...
def cancel_gl_entries(voucher_type, voucher_no):
	entries = frappe.get_all(
		...
		fields=["name", "organization", "posting_date", "account", "debit", "credit", "party_type", "party", "against", "remarks"],
	)
	if not entries:
		return
