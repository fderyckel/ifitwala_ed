...
class StaffCalendar(Document):
	...
	def validate_duplicate_date(self):
		...
		for day in self.holidays:
			if day.holiday_date in unique_dates:
				frappe.throw(
					_("Date {0} is duplicated. Please remove the duplicate date.").format(
	...
	def validate_break_values(self):
		...
		if (
			not (getdate(self.from_date) <= getdate(self.start_of_break) <= getdate(self.to_date))
			or not (getdate(self.from_date) <= getdate(self.end_of_break) <= getdate(self.to_date))
		):
