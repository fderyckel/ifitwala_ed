...
<template>
  <div class="space-y-6">
    ...
    <div v-else class="grid gap-4">
      ...
      <div v-if="!displayDocuments.length" class="rounded-2xl border border-border/70 bg-white px-4 py-4">
        <p class="type-caption text-ink/60">{{ __('No document requests are configured yet.') }}</p>
      </div>
    </div>
  </div>
</template>
...
function statusLabelFor(doc: ApplicantDocument | null, isRequired: boolean) {
  ...
  return doc.review_status === 'Approved' ? __('Approved') : doc.review_status === 'Rejected' ? __('Rejected') : __('Pending review')
}
