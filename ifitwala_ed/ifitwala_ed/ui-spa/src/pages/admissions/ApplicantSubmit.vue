...
<template>
  <div class="space-y-6">
    ...
    <div v-else class="space-y-4">
      ...
      <div v-if="blockingMessage" class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
...
const readinessItems = computed(() => {
  ...
  return items.map((item) => ({
    ...item,
    dot: item.state === 'complete' ? 'bg-leaf' : item.state === 'in_progress' ? 'bg-sun' : 'bg-amber-300',
  }))
})
