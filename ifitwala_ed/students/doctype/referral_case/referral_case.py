...
def quick_update_status(name: str, new_status: str):
    ...
    _ensure_case_action_permitted(doc)                       # ← guard
...
def add_entry(name: str, entry_type: str, summary: str, assignee: str | None = None,
    ...
    if "author" in doc.meta.get_field("entries").options:    # safe check
        try:
    ...
    if assignee and cint(create_todo):
        clean = strip_html(summary or "")[:120]              # ← strip HTML
        payload = {
...
def users_with_role(doctype, txt, searchfield, start, page_len, filters):
	...
	return frappe.db.sql("""
		...
		FROM `tabUser` u
		WHERE u.enabled = 1
		  AND u.name IN (SELECT parent FROM `tabHas Role` WHERE role=%s)
