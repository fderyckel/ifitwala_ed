...
from frappe.utils import add_to_date, cint, getdate, now_datetime, nowdate
...
class StudentReferral(Document):
    ...
    def validate(self):
        ...
        if cint(self.mandated_reporting_triggered):
            if self.confidentiality_level not in ("Sensitive", "Restricted"):
                frappe.throw(
                    _(
                        "When 'Mandated Reporting Triggered' is checked, confidentiality must be Sensitive or Restricted."
                    )
                )
    ...
    def before_submit(self):
        ...
        needs_case = (
            cint(self.mandated_reporting_triggered)
    ...
    def on_submit(self):
        ...
        if intake_role in roles:
            manager = author
            assign_case_manager(
                case_name, manager, description="Self-assigned (intake role author)", priority=priority, notify=notify
            )
        else:
            manager = pick_user_from_role_pool(intake_role)
            assign_case_manager(
                case_name, manager, description="Auto-routed from referral", priority=priority, notify=notify
            )
    ...
    def get_student_active_enrollment(self, student: str | None = None, on_date: str | None = None):
        ...
        if cand_unarch_within:
            ...
            return [
                {"name": r.name, "program": r.program, "academic_year": r.academic_year, "school": r.school}
        ...
        if len(rows) == 1:
            ...
            return {"name": r.name, "program": r.program, "academic_year": r.academic_year, "school": r.school}
        return [
            {"name": r.name, "program": r.program, "academic_year": r.academic_year, "school": r.school} for r in rows
        ]
...
def assign_case_manager(
    case_name: str, user: str, description: str = "Primary owner", priority: str = "Medium", notify: bool = True
):
...
def pick_user_from_role_pool(role: str) -> str:
    ...
    enabled = (
        set(frappe.get_all("User", filters={"name": ["in", users], "enabled": 1}, pluck="name")) if users else set()
    )
    if enabled:
