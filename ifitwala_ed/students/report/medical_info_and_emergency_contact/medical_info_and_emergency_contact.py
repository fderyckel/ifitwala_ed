...
def _render_guardian_block(child_row: frappe._dict, guardian_doc: frappe._dict | None) -> str:
	...
	details = []
	if phones:
		details.append(f"<div class='gc-line'><span class='gc-label'>{escape_html(_('Phone'))}:</span> {escape_html('; '.join(phones))}</div>")
	if emails:
		details.append(f"<div class='gc-line'><span class='gc-label'>{escape_html(_('Email'))}:</span> {escape_html('; '.join(emails))}</div>")
	if guardian_doc and guardian_doc.work_place:
		details.append(f"<div class='gc-line'><span class='gc-label'>{escape_html(_('Work'))}:</span> {escape_html(guardian_doc.work_place)}</div>")
