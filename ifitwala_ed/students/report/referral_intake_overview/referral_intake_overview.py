...
def execute(filters=None):
    ...
    if f.get("referral_category"):
        ...
        params["cats"] = (
            tuple(frappe.parse_json(f.referral_category))
            if isinstance(f.referral_category, str)
            else tuple(f.referral_category)
        )

    if f.get("referral_source"):
        cond.append("r.referral_source IN %(srcs)s")
        params["srcs"] = (
            tuple(frappe.parse_json(f.referral_source))
            if isinstance(f.referral_source, str)
            else tuple(f.referral_source)
        )
    ...
    if not can_see_restricted:
        cond.append(
            "(r.confidentiality_level != 'Restricted' OR r.assigned_case_manager = %(user)s OR r.referrer = %(user)s)"
        )
        params["user"] = user
