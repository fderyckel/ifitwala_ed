...
def _build_where(f, date_from, date_to, site_tz):
	...
	if date_from:
		where.append(f"{ENTRY_LOCAL_EXPR} >= %(df)s"); params["df"] = date_from  # noqa: E701, E702
	if date_to:
		where.append(f"{ENTRY_LOCAL_EXPR} <= %(dt)s"); params["dt"] = date_to  # noqa: E701, E702
	if f.get("student"):       where.append("rc.student = %(student)s");     params["student"] = f.student  # noqa: E701, E702
	if f.get("referral"):      where.append("rc.referral = %(referral)s");   params["referral"] = f.referral  # noqa: E701, E702
	if f.get("school"):        where.append("rc.school = %(school)s");       params["school"] = f.school  # noqa: E701, E702
	if f.get("program"):       where.append("rc.program = %(program)s");     params["program"] = f.program  # noqa: E701, E702
	if f.get("academic_year"): where.append("rc.academic_year = %(ay)s");    params["ay"] = f.academic_year  # noqa: E701, E702
	if f.get("case_manager"):  where.append("rc.case_manager = %(cm)s");     params["cm"] = f.case_manager  # noqa: E701, E702
	if f.get("assignee"):      where.append("rce.assignee = %(assignee)s");  params["assignee"] = f.assignee  # noqa: E701, E702
	if f.get("entry_type"):    where.append("rce.entry_type = %(et)s");      params["et"] = f.entry_type  # noqa: E701, E702
	if f.get("entry_status"):  where.append("rce.status = %(es)s");          params["es"] = f.entry_status  # noqa: E701, E702
	if f.get("case_severity"): where.append("rc.severity = %(sev)s");        params["sev"] = f.case_severity  # noqa: E701, E702
	if f.get("case_status"):   where.append("rc.case_status = %(cs)s");      params["cs"] = f.case_status  # noqa: E701, E702
	return where, params
