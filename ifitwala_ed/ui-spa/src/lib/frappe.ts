...
function normalizeRequestUrl(url: string): string {
	if (!url) {
		throw new Error(__('[frappe] Request url is required'));
	}
...
function extractMessagesFromPayload(payload: unknown): string[] {
	...
	const messages = [
		...
		typeof maybe._error_message === 'string' && maybe._error_message.trim()
			? maybe._error_message.trim()
			: '',
	];
...
function unwrapTransport<T>(res: unknown): T {
	...
	if (root === null || root === undefined) {
		throw new Error(
			__('[frappe] Invalid response shape: expected { message: T } or T (got null/undefined)')
		);
	}
	...
	if (typeof root === 'object') {
		if ('exc' in root || 'exception' in root) {
			throw new Error(__('[frappe] Server exception response (missing message envelope)'));
		}
	}
