...
import { Button, FormControl, Badge, FeatherIcon, Spinner, call, toast, createResource } from 'frappe-ui'
...
const programOptions = computed(() => {
	...
	return Array.from(progs).sort().map(p => ({ label: p, value: p }))
})
...
const hasActiveFilters = computed(() => {
	return !!(filters.school || filters.academic_year || filters.program || filters.course || groupSearch.value)
})
...
function onCriterionLevelChanged(taskStudent: string, criterionRow: CriterionPayload, levelValue: string) {
	const state = studentStates[taskStudent]
	if (!state) return
	const item = state.criteria.find((c) => c.assessment_criteria === criterionRow.assessment_criteria)
	if (!item) return
...
async function saveCriteria(taskStudent: string) {
	...
	try {
		const updates = state.criteria.map(c => ({
