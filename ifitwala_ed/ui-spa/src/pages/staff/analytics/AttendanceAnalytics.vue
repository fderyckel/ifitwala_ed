		...
		series: [
			{
				...
				data: points.map(point =>
					point.expected > 0 ? Number(((point.present / point.expected) * 100).toFixed(2)) : 0
				),
			},
		],
	};
...
const contextStudentOptions = computed(() => {
	...
	for (const row of risk.value?.frequent_unexplained || [])
		options.set(row.student, row.student_name);
	return Array.from(options.entries()).map(([student, student_name]) => ({
		student,
		student_name,
	}));
});
...
function buildBasePayload(): Omit<AttendanceBaseParams, 'mode'> {
	...
	if (!filters.start_date && !filters.end_date && preset.value !== 'term')
		payload.date_preset = preset.value;
...
async function reloadDashboard() {
	...
	try {
		...
		if (roleClass.value === 'instructor') {
			const myGroupsResponse = await analyticsService.getMyGroups({
				mode: 'my_groups',
				...basePayload,
			});
			if (runId !== loadRunId) return;
				...
				>
					<template #body>
						<div v-if="myGroups?.groups?.length" class="grid gap-3 md:grid-cols-2">
							<article
								v-for="group in myGroups.groups"
								:key="group.student_group"
								class="rounded-xl border border-slate-200 bg-surface-soft px-3 py-3"
							>
								...
								<div class="mt-2 grid grid-cols-4 gap-2 text-xs text-slate-700">
									<div>
										<p class="text-slate-500">Expected</p>
					...
					<template #body>
						<div v-if="myGroups?.emerging_patterns?.length" class="max-h-72 overflow-auto">
							<table class="w-full text-left text-xs">
								...
								<tbody>
									<tr
										v-for="row in myGroups.emerging_patterns.slice(0, 40)"
										:key="row.student"
				...
				<AnalyticsCard title="Today / This Week Exceptions" :interactive="false">
					<template #body>
						<ul
							...
							class="max-h-72 space-y-2 overflow-auto text-xs"
						>
							<li
								v-for="item in myGroups.exceptions.slice(0, 40)"
								:key="`${item.student_group}:${item.student}`"
				...
				<AnalyticsCard title="Back on Track" :interactive="false">
					<template #body>
						<ul v-if="myGroups?.improving_trends?.length" class="space-y-2 text-xs">
							<li
								v-for="item in myGroups.improving_trends.slice(0, 20)"
								:key="item.student"
								class="rounded-lg border border-leaf/30 bg-leaf/10 px-2 py-2"
							>
				...
				<AnalyticsCard title="Chronic Absence Radar" :interactive="false">
					<template #body>
						<AnalyticsChart
							...
							:option="riskBucketsOption"
							@click="onRiskRadarClick"
						/>
						<p
							v-if="Object.keys(riskBucketsOption).length"
				...
				<template v-if="isCounselor">
					<AnalyticsCard title="Top Critical" :interactive="false">
						<template #body>
							...
							>
								<li
									v-for="item in risk.top_critical.slice(0, 30)"
									:key="item.student"
									class="rounded-lg border border-slate-200 px-2 py-2"
								>
					...
					<AnalyticsCard title="Block vs Day Mismatch" :interactive="false">
						<template #body>
							...
							>
								<li
									v-for="item in risk.mismatch_students.slice(0, 30)"
									:key="item.student"
									class="rounded-lg border border-slate-200 px-2 py-2"
								>
									<p class="font-medium text-ink">{{ item.student_name }}</p>
					...
					<AnalyticsCard title="Back on Track" :interactive="false">
						<template #body>
							<ul v-if="risk?.improving_trends?.length" class="space-y-2 text-xs">
								<li
									v-for="item in risk.improving_trends.slice(0, 20)"
									:key="item.student"
									class="rounded-lg border border-leaf/30 bg-leaf/10 px-2 py-2"
								>
									<p class="font-medium text-leaf">{{ item.student_name }}</p>
			...
			>
				<template #body>
					<div class="flex flex-wrap items-end gap-3">
						...
						<button
							type="button"
							class="rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700"
					...
					<template #body>
						<div v-if="risk?.compliance_by_scope?.length" class="max-h-72 overflow-auto">
							<table class="w-full text-left text-xs">
								...
								<tbody>
									<tr
										v-for="row in risk.compliance_by_scope"
										:key="`${row.school}:${row.program}`"
										class="border-t border-slate-100"
									>
					...
					<template #body>
						<div v-if="codeUsage?.codes?.length" class="max-h-72 overflow-auto">
							<table class="w-full text-left text-xs">
								...
								<tbody>
									<tr
										v-for="row in codeUsage.codes.slice(0, 80)"
										:key="row.attendance_code"
										class="border-t border-slate-100"
									>
