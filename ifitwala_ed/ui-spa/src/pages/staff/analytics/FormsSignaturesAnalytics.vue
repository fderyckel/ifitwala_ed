	...
	<div class="analytics-shell space-y-5">
		<header class="page-header">
			<div class="page-header__intro">
				<h1 class="type-h1 text-canopy">{{ __('Forms & Signatures') }}</h1>
				<p class="type-meta text-slate-token/80">
					{{
						__(
							'Track operational forms that guardians or adult students complete in portal, while Desk remains the place where staff build and publish them.'
						)
					}}
				</p>
			...
			<div class="page-header__actions">
				<a href="/app/family-consent-request" class="if-button if-button--primary">
					...
					<span>{{ __('Open Desk authoring') }}</span>
				</a>
			</div>
		</header>
		...
		<FiltersBar class="analytics-filters !items-start">
			<div class="flex flex-col gap-1">
				...
				>
					<option value="">{{ __('All scoped organizations') }}</option>
					<option
			...
			<div class="flex flex-col gap-1">
				<label class="type-label">{{ __('Request Type') }}</label>
				<select
					v-model="filters.request_type"
					class="h-9 min-w-[210px] rounded-md border px-2 text-sm"
					@change="handleFilterChange"
				>
					<option value="">{{ __('All request types') }}</option>
					<option
			...
			<div class="flex flex-col gap-1">
				<label class="type-label">{{ __('Status') }}</label>
				<select
			...
			<div class="flex flex-col gap-1">
				<label class="type-label">{{ __('Completion Channel') }}</label>
				<select
		...
		<div v-if="accessDenied" class="rounded-md border border-amber-200 bg-amber-50 px-4 py-3">
			...
			<p class="mt-1 text-xs text-amber-800">
				{{ __('You do not have permission to view Forms & Signatures analytics.') }}
			</p>
		</div>
		...
		<div v-else class="space-y-5">
				...
				<div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
					<div class="space-y-1">
						...
						<h2 class="type-h3 text-ink">
							{{ __('Desk builds. Portal completes. Analytics stay here.') }}
						</h2>
						<p class="analytics-card__meta">
							{{
								__(
									'Use Desk to create and publish the request. Use this page to monitor portal and paper completion without opening the DocType list every time.'
								)
							}}
						</p>
					</div>
					<div class="flex flex-wrap gap-2">
						<span class="rounded-full bg-slate-100 px-3 py-1 type-caption text-slate-700">
							{{ __('Portal + paper-aware monitoring') }}
						</span>
			...
			>
				<span class="font-medium">{{ __('Paper Only') }}</span>
				{{
					__(
						'requests stay visible for monitoring here, but families cannot submit them from the portal. Staff must capture those approvals in Desk.'
					)
				}}
			</section>
			...
			<section class="analytics-card space-y-4">
				<div class="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
					<div>
						<h2 class="type-h3 text-ink">{{ __('Request tracking') }}</h2>
						<p class="analytics-card__meta">
							{{
								__(
									"Completion counts roll up across each request's targeted students so staff can see pending, completed, overdue, and exception states in one board."
								)
							}}
						</p>
					</div>
					<p class="type-caption text-slate-500">
						{{ __('Updated {0}', [generatedAtLabel]) }}
					</p>
				</div>
				...
				>
					{{ __('Refreshing request analytics...') }}
				</div>
				...
				<div
					...
					class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-6 text-center text-sm text-slate-500"
				>
					{{ __('No requests match the current scope and filters.') }}
				</div>
					...
					<table class="w-full min-w-[1120px] text-sm">
						<thead>
							<tr class="border-b border-slate-200 text-left text-slate-500">
								<th class="pb-3 pr-4 font-medium">{{ __('Request') }}</th>
								<th class="pb-3 pr-4 font-medium">{{ __('Audience') }}</th>
								<th class="pb-3 pr-4 font-medium">{{ __('Channel') }}</th>
								<th class="pb-3 pr-4 font-medium">{{ __('Scope') }}</th>
								<th class="pb-3 pr-4 font-medium">{{ __('Due') }}</th>
								<th class="pb-3 pr-4 text-right font-medium">{{ __('Targets') }}</th>
								<th class="pb-3 pr-4 text-right font-medium">{{ __('Completed') }}</th>
								<th class="pb-3 pr-4 text-right font-medium">{{ __('Pending') }}</th>
								<th class="pb-3 pr-4 text-right font-medium">{{ __('Exceptions') }}</th>
								<th class="pb-3 text-right font-medium">{{ __('Completion') }}</th>
							</tr>
						</thead>
						<tbody>
							<tr
							...
							>
								...
								<td class="py-3 pr-4">
									<div class="flex flex-col gap-1">
										...
										<span class="type-caption text-slate-500">
											{{ row.school || __('All schools in scope') }}
										</span>
									</div>
								</td>
...
const scopeLabel = computed(() => {
	...
	return __('All scoped organizations');
});

const requestCountLabel = computed(() => {
	const count = dashboard.value?.counts.requests || 0;
	return count === 1 ? __('1 request in view') : __('{0} requests in view', [count]);
});
	...
	return [
		{
			...
			subLabel: __('Published, closed, and archived requests in current scope'),
		},
		{
			id: 'completed',
			label: __('Completed'),
			value: counts.completed,
			subLabel: __('Students with a current approved or granted decision'),
		},
		{
			id: 'pending',
			label: __('Pending'),
			value: counts.pending,
			subLabel: __('Still waiting for portal or paper completion'),
		},
		{
			id: 'exceptions',
			label: __('Exceptions'),
			value: exceptionTotal,
			subLabel: __('Declined, withdrawn, overdue, or expired targets'),
		},
	];
});
...
const channelCards = computed(() => {
	...
	return [
		...
		},
		{
			...
			badge: __('Offline'),
			meta: __('Portal shows status only; staff record collected approvals in Desk.'),
		},
	];
});
...
async function refreshContext() {
	...
	try {
		...
		setErrorState(err, __('Unable to load forms and signatures filter options.'));
	} finally {
...
async function refreshDashboard() {
	...
	try {
		...
		setErrorState(err, __('Unable to load forms and signatures analytics.'));
	} finally {
...
function formatDateValue(value?: string | null) {
	const normalized = String(value || '').trim();
	if (!normalized) return __('No due date');
...
function formatDateTimeValue(value: string) {
	const normalized = String(value || '').trim();
	if (!normalized) return __('just now');
...
function channelMeta(channel: FamilyConsentCompletionChannelMode) {
	if (channel === 'Portal Only') return __('Portal submission required');
	if (channel === 'Portal Or Paper') return __('Portal or paper accepted');
	return __('Paper completion only');
}
