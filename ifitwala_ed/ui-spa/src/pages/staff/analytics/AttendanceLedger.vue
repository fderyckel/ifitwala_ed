		...
		.map(row =>
			columns.value
				.map(column => {
					...
					if (
						(column.fieldname === 'percentage_present' ||
							column.fieldname === 'percentage_late') &&
						raw !== null &&
						raw !== undefined &&
						raw !== ''
					) {
				...
				<template #body>
					<div class="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-600">
						...
						<div class="flex items-center gap-2">
							...
							>
								<option v-for="size in pageLengthOptions" :key="size" :value="size">
									{{ size }}
								</option>
							</select>
						</div>
							...
							<thead class="text-slate-500">
								<tr>
									...
									>
										...
										>
											<span>{{ col.label }}</span>
											<span v-if="sortBy === col.fieldname">{{
												sortOrder === 'asc' ? '↑' : '↓'
											}}</span>
										</button>
									</th>
							...
							<tbody>
								...
								>
									...
									>
										...
										<span
											v-else-if="col.fieldname === 'percentage_present'"
											:class="[
												'rounded-full px-2 py-0.5 text-[11px] font-medium',
												Number(row[col.fieldname] || 0) >= 95
													? 'bg-leaf/15 text-leaf'
													: Number(row[col.fieldname] || 0) < 75
										...
										<span
											v-else-if="col.fieldname === 'percentage_late'"
