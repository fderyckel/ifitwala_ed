...
// When any filter changes → refresh (debounced)
watch(
  filters,
  () => {
    debounce(() => {
...
// Utility: format date and truncate text a bit
...
onMounted(() => {
  // optional: later we can fetch default school / AY from a small API
  loadDashboard()
  loadRecent(true)
})
</script>
...
<template>
  <div class="min-h-full px-4 py-4 md:px-6 lg:px-8">
    ...
    >
      <!-- Simple Link-like filters. Replace with your own Link controls later if you want. -->
    ...
    <section class="mt-4 grid gap-4 xl:grid-cols-[2fr,1.6fr]">
      <!-- Left column: trends & breakdowns -->
      <div class="flex flex-col gap-4">
        <!-- Incidents over time -->
        <AnalyticsCard title="Incidents Over Time">
          <template #body>
            <SimpleList
              :items="incidentsOverTime"
              label-key="label"
              value-key="value"
              empty-label="No logs for this period."
            />
            <!-- Later: replace this with a line chart component using the same data -->
          </template>
        </AnalyticsCard>

        <!-- Log type + next step types side by side -->
        <div class="grid gap-4 md:grid-cols-2">
          <AnalyticsCard title="Log Types">
            <template #body>
              <SimpleListPercent
                :items="logTypeCount"
                empty-label="No logs found."
              />
            </template>
          </AnalyticsCard>
          ...
          <AnalyticsCard title="Next Step Types">
            <template #body>
              <SimpleListPercent
                :items="nextStepTypes"
                empty-label="No next steps recorded."
              />
            </template>
          </AnalyticsCard>
        </div>
...
<!-- Reusable analytics card + list components -->
<script lang="ts">
export default {}
</script>

<script setup lang="ts">
// local components declared in same file for now (easy to extract later)

/**
 * Generic analytics card – reusable for all future dashboards.
 */
