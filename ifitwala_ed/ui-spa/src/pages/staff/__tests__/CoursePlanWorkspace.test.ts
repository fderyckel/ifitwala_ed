...
describe('CoursePlanWorkspace page', () => {
	...
	it('does not show the retired legacy standards warning banner', async () => {
		getStaffCoursePlanSurfaceMock.mockResolvedValue({
			meta: {
				generated_at: '2026-04-13 10:00:00',
				course_plan: 'COURSE-PLAN-1',
		...
		expect(document.body.textContent || '').not.toContain('needs re-selection');
		expect(document.body.textContent || '').not.toContain(
			'approved learning-standards master'
		);
	});
