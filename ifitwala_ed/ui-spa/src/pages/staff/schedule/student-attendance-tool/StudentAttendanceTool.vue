	...
	<div class="attendance-shell mx-auto flex h-full max-w-7xl flex-col gap-6 p-4 pb-10">
		...
		<div class="grid gap-6 xl:grid-cols-[minmax(0,3fr)_minmax(280px,1fr)]">
			<section class="attendance-panel flex flex-col overflow-hidden rounded-2xl shadow-sm">
				<div class="flex flex-wrap items-center justify-between gap-3 border-b border-border/60 bg-white/60 px-5 py-4">
					<div class="min-w-0">
						...
						<p v-if="filters.student_group && groupSubtitle" class="text-xs uppercase tracking-wide text-ink/60">
							{{ groupSubtitle }}
						</p>
				...
				<div class="flex flex-wrap items-center gap-3 border-b border-border/60 bg-white/50 px-5 py-3">
					<FormControl
						v-model="searchTerm"
						type="text"
				...
				<div class="flex-1 overflow-hidden">
					...
					<div v-else-if="calendarLoading" class="flex h-full flex-col items-center justify-center gap-3 py-12 text-ink/70">
					...
					>
						...
						<p class="text-base font-semibold text-ink">{{ __('No scheduled meetings found for this group.') }}</p>
					...
					>
						...
						<p class="text-base font-semibold text-ink">{{ __('Select a date from the calendar to record attendance.') }}</p>
						<p class="max-w-sm text-sm text-ink/70">
					...
					<div v-else class="flex h-full flex-col">
						<div v-if="rosterLoading" class="flex h-full flex-col items-center justify-center gap-3 py-12 text-ink/70">
							<Spinner class="h-6 w-6" />
						...
						>
							...
							<p class="text-base font-semibold text-ink">{{ __('No students found in this group.') }}</p>
						</div>
						...
						<div v-else-if="isMeetingDay" class="flex h-full flex-col overflow-hidden">
							...
							>
								...
								<p class="text-base font-semibold text-ink">{{ __('No students match your search.') }}</p>
							...
							<div class="border-t border-border/60 bg-white/50 px-5 py-3 text-xs text-ink/70">
						...
						>
							...
							<p class="text-base font-semibold text-ink">{{ __('This group is not scheduled to meet on this date.') }}</p>
							<p class="max-w-sm text-sm text-ink/70">
			...
			<aside class="flex flex-col gap-4">
				...
				<div class="attendance-card rounded-2xl p-4 shadow-sm">
					...
					<div class="mt-3 flex flex-wrap gap-2">
						...
						>
							...
							<span class="text-ink/80">{{ code.attendance_code_name || code.name }}</span>
						</div>
					</div>
				...
				<div v-if="filters.student_group" class="attendance-card rounded-2xl p-4 shadow-sm">
					<h3 class="text-sm font-semibold text-ink">
						{{ __('Group details') }}
					</h3>
					<dl class="mt-3 space-y-2 text-sm text-ink/80">
						<div v-if="groupInfo.name" class="flex justify-between gap-2 text-ink">
							...
							<dd class="font-medium text-right text-ink">{{ groupInfo.name }}</dd>
						</div>
    ...
    >
      <template #body>
        <div class="prose prose-sm max-w-none text-ink">
          ...
          <p v-else class="text-ink/70">{{ __('No medical information available.') }}</p>
        </div>
      </template>
    </Dialog>
...
.attendance-card {
	background: linear-gradient(180deg, rgba(var(--sky-rgb), 0.6), #fff);
	border: 1px solid rgba(var(--border-rgb), 0.9);
	box-shadow: var(--shadow-soft);
}
</style>
