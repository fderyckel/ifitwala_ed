...
<template>
	<div class="flex h-full flex-col text-ink">
		<!-- Header row (desktop) -->
		<div
			class="hidden border-b border-border/70 bg-[rgb(var(--surface-rgb)/0.8)] px-5 py-3 text-xs font-semibold uppercase tracking-wide text-ink/60 md:grid md:grid-cols-[minmax(220px,1.05fr)_minmax(0,1.6fr)_88px]"
		>
		...
		>
			<div
				class="flex flex-col gap-4 md:grid md:grid-cols-[minmax(220px,1fr)_minmax(0,1.5fr)] md:items-start md:gap-4"
			>
				<!-- Identity -->
				<div class="flex min-w-0 items-center gap-3 md:min-w-[220px]">
					<a
						:href="studentLink(student.student)"
						target="_blank"
						rel="noopener"
						class="relative inline-flex h-16 w-16 flex-shrink-0 overflow-hidden rounded-xl ring-1 ring-border/70"
					>
						<img
					...
					<div class="min-w-0 space-y-1">
						<div class="flex flex-wrap items-center gap-2">
							<a
								...
								target="_blank"
								rel="noopener"
								class="truncate text-base font-semibold text-ink hover:text-jacaranda"
							>
								{{ displayName(student) }}
							</a>
							<span v-if="student.preferred_name" class="truncate text-sm text-ink/60">
				...
				<div class="flex min-w-0 flex-col gap-2">
					<div
						...
						class="flex flex-col gap-3 rounded-xl border border-border/50 bg-[rgb(var(--surface-rgb)/0.9)] p-3 shadow-inner sm:flex-row sm:items-center sm:justify-between md:rounded-none md:border-0 md:bg-transparent md:p-0 md:shadow-none"
					>
						<div class="flex flex-wrap items-center gap-2 md:min-w-[72px] md:flex-none">
						...
						<div
							class="flex w-full min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between md:flex-1 md:justify-start md:gap-3"
						>
							<div class="flex flex-wrap items-center gap-2">
								<button
									v-for="code in codes"
									:key="code.name"
									type="button"
									class="inline-flex items-center justify-center rounded-full border px-3 py-1 text-xs font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-jacaranda disabled:cursor-not-allowed"
									:class="chipClass(student.attendance[block] === code.name)"
									:style="chipStyle(code, student.attendance[block] === code.name)"
									:disabled="disabled"
									@click="
										$emit('change-code', { studentId: student.student, block, code: code.name })
									"
							...
							<div class="flex items-center gap-2 md:ml-auto md:flex-none">
								<Button
									appearance="minimal"
									icon="message-circle"
								...
								/>
								<p v-if="student.remarks?.[block]" class="hidden text-xs text-ink/60 lg:inline">
			...
			<div
				...
				class="mt-3 rounded-xl border border-jacaranda/20 bg-jacaranda/5 px-4 py-3 text-sm text-ink/80 md:hidden"
			>
...
const popoverBody = computed(() => {
	...
	if (popover.value.type === 'health') {
		const txt = stripHtml(s.medical_info || '')
			.replace(/&nbsp;/gi, ' ')
			.replace(/\s+/g, ' ')
			.trim();
...
function chipClass(isSelected: boolean) {
	return isSelected
		...
		: 'bg-white text-ink/70 hover:bg-jacaranda/5 hover:text-jacaranda';
}
