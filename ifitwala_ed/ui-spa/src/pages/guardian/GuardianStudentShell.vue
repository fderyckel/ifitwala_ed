...
<template>
	<div class="space-y-6">
			...
			<div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
				<div>
					...
					<p class="type-body text-ink/70">
						{{ selectedChild?.school || 'School unavailable' }} · {{ studentId || 'Unknown student' }}
					</p>
				</div>
				<div class="flex items-center gap-2">
					<RouterLink :to="{ name: 'guardian-home' }" class="if-action">Back to Family Snapshot</RouterLink>
					<button type="button" class="if-action" :disabled="loading" @click="loadSnapshot">Refresh</button>
				</div>
			</div>
		</header>
		...
		<section v-else-if="!selectedChild" class="card-surface p-5">
			<p class="type-body-strong text-flame">This student is not available in your guardian scope.</p>
		</section>

		<template v-else>
			<section class="card-surface p-5">
				<h2 class="mb-3 type-h3 text-ink">Timeline</h2>
				<div v-if="!studentTimeline.length" class="type-body text-ink/70">No timeline items in this window.</div>
				<div v-else class="space-y-3">
					...
					>
						...
						<p class="type-caption text-ink/60">
							{{ day.child.day_summary.start_time || '--:--' }} - {{ day.child.day_summary.end_time || '--:--' }}
						</p>
						...
						<p v-else class="mt-2 type-caption text-ink/60">{{ day.child.day_summary.note || 'No scheduled blocks' }}</p>

						<div v-if="day.child.tasks_due.length || day.child.assessments_upcoming.length" class="mt-2 space-y-1">
			...
			<section class="card-surface p-5">
				...
				<div v-if="!studentAttention.length" class="type-body text-ink/70">No attention items right now.</div>
			...
			<section class="card-surface p-5">
				...
				<div v-if="!studentPrep.length" class="type-body text-ink/70">No preparation cues in this window.</div>
				<ul v-else class="space-y-2">
					<li
...
const studentPrep = computed<PrepItem[]>(() =>
	(snapshot.value?.zones.preparation_and_support ?? []).filter((item) => item.student === studentId.value)
)
