									...
									>
										<div class="mb-2 flex items-center justify-between gap-2">
											<label class="inline-flex items-center gap-2 type-body-strong text-ink">
												<input
													v-model="
														familySelection[offering.program_offering][student.student].enabled
													"
													type="checkbox"
...
const batchResult = ref<
	Record<string, Array<{ ok: boolean; student?: string; error?: string | null }>>
>({});
...
const waitlistCount = computed<number>(() =>
	students.value.reduce(
		(total, student) => total + student.bookings.filter(row => row.status === 'Waitlisted').length,
		0
	)
);
...
function rankSlots(offeringKey: string, student: string): string[] {
	const sectionNames =
		offerings.value
			.find(row => row.program_offering === offeringKey)
			?.sections.map(row => row.student_group) || [];
