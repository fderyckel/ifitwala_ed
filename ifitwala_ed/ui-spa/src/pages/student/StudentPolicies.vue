...
const overview = ref<StudentPolicyOverview | null>(null);
...
function hasRequiredClausesChecked(row: StudentPolicyRow): boolean {
	const checked = new Set(selectedClauseNames(row.policy_version));
...
function updateTypedSignature(policyVersion: string, event: Event) {
	typedSignatureByVersion.value[policyVersion] = (event.target as HTMLInputElement).value;
	signatureTouched.value[policyVersion] = true;
}

function toggleAttestation(policyVersion: string, event: Event) {
	attestationByVersion.value[policyVersion] = (event.target as HTMLInputElement).checked;
}

function toggleClause(policyVersion: string, clauseName: string, event: Event) {
	const checked = (event.target as HTMLInputElement).checked;
	const next = new Set(selectedClauseNames(policyVersion));
	if (checked) next.add(clauseName);
	else next.delete(clauseName);
	checkedClausesByVersion.value[policyVersion] = Array.from(next);
}

function isTypedSignatureMatch(row: StudentPolicyRow): boolean {
...
async function acknowledgeRow(row: StudentPolicyRow) {
	const policyVersion = row.policy_version;
	submitAttempts.value[policyVersion] = true;
	signatureTouched.value[policyVersion] = true;
	...
	busyRows.value[policyVersion] = true;
	try {
		const result = await acknowledgeStudentPolicy({
