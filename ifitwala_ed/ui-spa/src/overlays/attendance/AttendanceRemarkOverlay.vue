						...
						<div class="if-overlay__body px-6 pb-2">
							...
							<div v-else class="space-y-3">
								<div class="flex flex-wrap items-center gap-2 text-xs">
									...
									>
										<span
											class="inline-block h-1.5 w-1.5 rounded-full bg-[rgb(var(--leaf-rgb))]"
										/>
										{{ studentLabel }}
									</span>
									<span
								...
								>
									<textarea
										...
										rows="4"
										:maxlength="maxLength"
										class="w-full rounded-xl border border-[var(--border-light)] bg-[rgb(var(--surface-strong-rgb))] px-3 py-2 text-sm text-ink shadow-inner focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[rgb(var(--leaf-rgb)/0.55)]"
										:placeholder="__('Add a short, specific note (optional)...')"
									/>

									<div
										class="mt-2 flex items-center justify-between text-[0.7rem] text-slate-token/60"
									>
...
const helperText = computed(() => {
	...
	if (props.blockNumber === null || props.blockNumber === undefined)
		return __('Add a remark once a block is selected.');
