						...
						<header class="interview-workspace__header px-6 pt-6">
							<div class="min-w-0">
								...
								<DialogTitle class="type-h2 text-canopy truncate">
									{{ applicantDisplayName || __('Applicant Workspace') }}
								</DialogTitle>
							...
							<div class="flex items-center gap-2">
								...
								<button
									...
									:aria-label="__('Close admissions workspace')"
								>
						...
						>
							...
							>
								<p class="type-body-strong text-rose-900">
									{{ __('Unable to load admissions workspace') }}
								</p>
								...
								<div class="mt-4 flex justify-end gap-2">
									...
									<button class="if-action" type="button" @click="loadWorkspace">
										{{ __('Retry') }}
									</button>
								</div>
							</div>
							...
							<div v-else-if="hasWorkspace" class="space-y-4">
								<section v-if="isGuardianMode && selectedGuardian" class="space-y-4">
												...
												<div class="min-w-0">
													<div class="flex flex-wrap items-center gap-2">
														<h3 class="type-h3 text-ink">
															{{ selectedGuardian.full_name || __('Guardian') }}
														</h3>
														<span
													...
													<p class="mt-1 type-caption text-ink/65">
														{{ __('Guardian intake details captured on the applicant profile.') }}
													</p>
												</div>
											</div>
										</div>
									...
									<article class="interview-card">
										<h3 class="type-h3 text-ink">{{ __('Profile') }}</h3>
									...
									<article class="interview-card">
										<h3 class="type-h3 text-ink">{{ __('Contact & Access') }}</h3>
										<div class="mt-3 grid gap-3 sm:grid-cols-2">
									...
									<article class="interview-card">
										<h3 class="type-h3 text-ink">{{ __('Work Details') }}</h3>
										<div class="mt-3 grid gap-3 sm:grid-cols-2">
								...
								<section v-else class="space-y-4">
									<article class="interview-card">
										<h3 class="type-h3 text-ink">{{ __('Applicant Brief') }}</h3>
										<div class="mt-3 grid gap-3 sm:grid-cols-2">
											<div>
												<p class="type-caption text-ink/65">{{ __('Status') }}</p>
											...
											<div>
												<p class="type-caption text-ink/65">{{ __('School') }}</p>
												<p class="type-body text-ink">{{ workspaceApplicant?.school || '—' }}</p>
											</div>
											<div>
												<p class="type-caption text-ink/65">{{ __('Applicant Email') }}</p>
												<p class="type-body text-ink">
													{{ workspaceApplicant?.applicant_email || '—' }}
												</p>
											</div>
											<div>
												<p class="type-caption text-ink/65">{{ __('Submitted') }}</p>
												<p class="type-body text-ink">
													{{ formatHumanDateTime(workspaceApplicant?.submitted_at) }}
												</p>
											</div>
											<div>
												<p class="type-caption text-ink/65">{{ __('Date of Birth') }}</p>
												<p class="type-body text-ink">
													{{
									...
									<article class="interview-card">
										<div class="flex items-center justify-between gap-2">
											<h3 class="type-h3 text-ink">{{ __('Guardians') }}</h3>
											<span
										...
										>
											{{ __('No guardian details available.') }}
										</div>
									...
									<article class="interview-card">
										<div class="flex items-center justify-between gap-2">
											<h3 class="type-h3 text-ink">{{ __('Interviews') }}</h3>
											<span
												class="type-badge-label rounded-full bg-sky/20 px-2 py-0.5 text-ink/70"
											>
										...
										<div v-if="!workspaceInterviews.length" class="mt-2 type-caption text-ink/60">
											{{ __('No interviews recorded yet for this applicant.') }}
										</div>
												...
												<div class="flex items-start justify-between gap-3">
													<div class="min-w-0">
														<p class="type-body-strong text-ink truncate">
															{{ item.interview_type || __('Interview') }}
													...
													>
														{{ isActiveInterview(item.name) ? __('Opened') : __('Open') }}
													</button>
												</div>
											</li>
										</ul>
									...
									<template v-if="isInterviewMode && workspace">
										<article class="interview-card">
											<h3 class="type-h3 text-ink">{{ __('Interview Context') }}</h3>
											<div class="mt-3 grid gap-3 sm:grid-cols-2">
												<div>
													<p class="type-caption text-ink/65">{{ __('Interview Type') }}</p>
													<p class="type-body text-ink">
														{{ workspace.interview.interview_type || '—' }}
													</p>
												</div>
												<div>
													<p class="type-caption text-ink/65">{{ __('Mode') }}</p>
													<p class="type-body text-ink">
														{{ workspace.interview.mode || '—' }}
													</p>
												</div>
												<div>
													<p class="type-caption text-ink/65">{{ __('Confidentiality') }}</p>
													<p class="type-body text-ink">
														{{ workspace.interview.confidentiality_level || '—' }}
													</p>
												</div>
												<div>
													<p class="type-caption text-ink/65">{{ __('Scheduled') }}</p>
													<p class="type-body text-ink">
											...
											>
												<p class="type-caption text-ink/65">{{ __('Operational Notes') }}</p>
										...
										<article class="interview-card">
											<h3 class="type-h3 text-ink">{{ __('Interview Team') }}</h3>
											<ul class="mt-3 space-y-2">
												...
												>
													...
													>
														{{ member.feedback_status || __('Pending') }}
													</span>
												</li>
											</ul>
										</article>
										...
										<article class="interview-card">
											...
											<div v-if="canEdit" class="mt-3 space-y-3">
												...
												<label class="block space-y-1">
													<span class="type-caption text-ink/70">{{ __('Strengths') }}</span>
													<textarea
												...
												<label class="block space-y-1">
													<span class="type-caption text-ink/70">{{ __('Concerns') }}</span>
													<textarea
												...
												<label class="block space-y-1">
													<span class="type-caption text-ink/70">{{ __('Shared Values') }}</span>
													<textarea
												...
												<label class="block space-y-1">
													<span class="type-caption text-ink/70">{{ __('Other Notes') }}</span>
													<textarea
									...
									<article v-else class="interview-card">
										...
										<p class="type-caption text-ink/70 mt-2">
											{{ __('Select an interview to view the team status and add your notes.') }}
										</p>
									</article>

									<article v-if="isInterviewMode" class="interview-card">
										<div class="flex items-center justify-between gap-2">
											<h3 class="type-h3 text-ink">{{ __('Documents') }}</h3>
											<span
												class="type-badge-label rounded-full bg-sky/20 px-2 py-0.5 text-ink/70"
											>
										...
										<div
											...
											class="mt-2 type-caption text-ink/60"
										>
											{{ __('No applicant documents found.') }}
										</div>
										<ul v-else class="mt-3 space-y-2 max-h-64 overflow-y-auto pr-1">
											...
											>
												...
												<p class="type-caption text-ink/70 mt-1">
													{{
														doc.items.length === 1
															? __('{0} file', [doc.items.length])
															: __('{0} files', [doc.items.length])
													}}
												</p>
													...
													>
														<button
															...
															@click="
																openWorkspaceFile(
																	item.open_url,
																	item.item_label || item.file_name || __('submitted file')
																)
															"
														>
															{{ item.item_label || item.file_name || __('View file') }}
														</button>
														<span v-else>{{
															item.item_label || item.item_key || __('Submitted file')
														}}</span>
													</li>
												</ul>
											</li>
									...
									<article v-else class="interview-card">
										<div class="flex items-start justify-between gap-3">
											<div>
												<h3 class="type-h3 text-ink">{{ __('Evidence Review') }}</h3>
												<p class="mt-1 type-caption text-ink/65">
													{{
														__(
											...
											>
												{{
													applicantRequirementRows.length === 1
														? __('{0} requirement', [applicantRequirementRows.length])
														: __('{0} requirements', [applicantRequirementRows.length])
												}}
											</span>
										</div>
										...
										<div class="mt-3 grid gap-2 sm:grid-cols-3">
											<div class="rounded-xl bg-sky/20 px-3 py-2">
												<p class="type-caption text-ink/70">{{ __('Complete') }}</p>
												<p class="type-body-strong text-ink">
													{{ completedRequirementCount }}/{{ applicantRequirementRows.length }}
												</p>
											</div>
											<div class="rounded-xl bg-sky/20 px-3 py-2">
												<p class="type-caption text-ink/70">{{ __('Missing Requirements') }}</p>
												<p class="type-body-strong text-ink">{{ missingRequirementCount }}</p>
											</div>
											<div class="rounded-xl bg-sky/20 px-3 py-2">
												<p class="type-caption text-ink/70">{{ __('Pending File Reviews') }}</p>
										...
										>
											{{ __('No document requirements are in scope for this applicant.') }}
										</div>
												...
												>
													<label class="block space-y-1">
														<span class="type-caption text-ink/70">
															...
															{{ requirementActionLabel || __('this requirement') }}
														</span>
														<textarea
													...
													<div class="mt-3 flex flex-wrap justify-end gap-2">
														...
														>
															{{ requirementSubmitting ? __('Saving…') : __('Apply Override') }}
														</button>
													</div>
												</div>
													...
													>
															...
															<div class="min-w-0">
																...
																<button
																	...
																	@click="
																		openWorkspaceFile(
																			item.open_url,
																			item.file_name || item.item_label || __('submitted file')
																		)
																	"
																>
																	{{ item.file_name || __('Open file') }}
																</button>
															</div>
														...
														>
															...
															<div class="mt-3 flex flex-wrap justify-end gap-2">
																...
																>
																	{{ submissionSubmitting ? __('Saving…') : __('Save Review') }}
																</button>
															</div>
														</div>
													</div>
												</div>
												<p v-else class="mt-3 type-caption text-ink/60">
													{{ __('No submitted files yet.') }}
												</p>
											</section>
										</div>
										...
										<div v-if="applicantSupplementalUploads.length" class="mt-4">
											<h4 class="type-body-strong text-ink">
												{{ __('Additional Submitted Files') }}
											</h4>
											<p class="mt-1 type-caption text-ink/65">
												{{
													__(
														'Optional evidence appears here so staff can review it without leaving applicant context.'
													)
												}}
											</p>
									...
									<article class="interview-card">
										<div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
											<div>
												<h3 class="type-h3 text-ink">{{ __('Recommendations') }}</h3>
												<p class="mt-1 type-caption text-ink/65">
													{{
														__(
															'Review referee context and approve the linked submission without leaving applicant workspace.'
														)
													}}
												</p>
											</div>
										...
										<div class="mt-3 grid gap-2 sm:grid-cols-4">
											<div class="rounded-xl bg-sky/20 px-3 py-2">
												<p class="type-caption text-ink/70">{{ __('Required') }}</p>
												<p class="type-body-strong text-ink">
													{{ workspaceRecommendations.summary.required_total || 0 }}
												</p>
											</div>
											<div class="rounded-xl bg-sky/20 px-3 py-2">
												<p class="type-caption text-ink/70">{{ __('Received') }}</p>
												<p class="type-body-strong text-ink">
													{{ workspaceRecommendations.summary.received_total || 0 }}
												</p>
											</div>
											<div class="rounded-xl bg-sky/20 px-3 py-2">
												<p class="type-caption text-ink/70">{{ __('Requested') }}</p>
												<p class="type-body-strong text-ink">
													{{ workspaceRecommendations.summary.requested_total || 0 }}
												</p>
											</div>
											<div class="rounded-xl bg-sky/20 px-3 py-2">
												<p class="type-caption text-ink/70">{{ __('Pending Review') }}</p>
												<p class="type-body-strong text-ink">
													{{ workspaceRecommendations.summary.pending_review_count || 0 }}
												</p>
											</div>
										...
										>
											{{ __('No submitted recommendations are available yet.') }}
										</div>
										<div v-else class="mt-4 grid gap-4 xl:grid-cols-[0.92fr_1.08fr]">
											<div class="space-y-2">
												...
												>
													...
													<p class="mt-2 type-caption text-ink/70">
														{{ __('Submitted {0}', [formatHumanMoment(row.submitted_on)]) }}
													</p>
													<p class="mt-1 type-caption text-ink/60">
														{{ __('Shared {0}', [formatHumanMoment(row.sent_on)]) }}
														<span v-if="row.opened_on">
															· {{ __('Opened {0}', [formatHumanMoment(row.opened_on)]) }}</span
														>
													</p>
												</button>
											</div>
											...
											<div class="rounded-2xl border border-border/70 bg-slate-50/60 p-4">
												...
												>
													<p class="type-body-strong text-rose-900">
														{{ __('Unable to load recommendation') }}
													</p>
												...
												<div v-else-if="recommendationReview" class="space-y-4">
													...
													>
														...
														>
															{{
																recommendationReview.recommendation.review_status || __('Pending')
															}}
														</span>
													</div>
													...
													<div class="grid gap-3 sm:grid-cols-2">
														<div class="rounded-xl border border-border/70 bg-white px-3 py-2">
															<p class="type-caption text-ink/60">{{ __('Shared') }}</p>
															<p class="type-body text-ink">
																{{
														...
														<div class="rounded-xl border border-border/70 bg-white px-3 py-2">
															<p class="type-caption text-ink/60">{{ __('Opened') }}</p>
															<p class="type-body text-ink">
																{{
														...
														<div class="rounded-xl border border-border/70 bg-white px-3 py-2">
															<p class="type-caption text-ink/60">{{ __('Submitted') }}</p>
															<p class="type-body text-ink">
																{{
																	formatHumanMoment(
														...
														<div class="rounded-xl border border-border/70 bg-white px-3 py-2">
															<p class="type-caption text-ink/60">{{ __('Reviewed') }}</p>
															<p class="type-body text-ink">
																{{
																...
																@click="
																	openWorkspaceFile(
																		...
																		recommendationReview.recommendation.file_name ||
																			__('recommendation attachment')
																	)
																"
															>
																{{
																	recommendationReview.recommendation.file_name ||
																	__('Open attached file')
																}}
															</button>
														</div>
														<p
													...
													<div>
														<h5 class="type-body-strong text-ink">
															{{ __('Submission Answers') }}
														</h5>
														<div
													...
													<p v-else-if="recommendationReviewItem" class="type-caption text-ink/60">
														{{
															__(
																'Evidence review actions are available from applicant workspace for admissions reviewers.'
															)
														}}
													</p>
												</div>
									...
									<article class="interview-card">
										<h3 class="type-h3 text-ink">{{ __('Timeline Highlights') }}</h3>
										<div v-if="!workspaceTimeline.length" class="mt-2 type-caption text-ink/60">
											{{ __('No timeline entries yet.') }}
										</div>
