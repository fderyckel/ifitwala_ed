				...
				<div class="flex items-center gap-3 type-caption">
					<button class="if-action type-button-label" @click="handleRefresh">
						<FeatherIcon
							name="refresh-cw"
							class="h-4 w-4"
							:class="{ 'animate-spin': calendarResource.loading }"
						/>
						Refresh
					</button>
				</div>
...
const activeSources = ref<Set<StudentCalendarSource>>(
	new Set(['student_group', 'meeting', 'school_event', 'holiday'])
);
...
const filteredEvents = computed(() =>
	allEvents.value.filter(evt => {
		const source =
			typeof evt === 'object' && evt && 'source' in evt
				? String((evt as Record<string, unknown>).source || '')
				: '';
...
const isEmpty = computed(
	() =>
		!calendarResource.loading && !calendarErrorMessage.value && filteredEvents.value.length === 0
);

const sourcePalette: Record<
	StudentCalendarSource,
...
function inferSourceFromId(rawId?: string | null): StudentCalendarSource | '' {
	...
	if (rawId.startsWith('sg::') || rawId.startsWith('sg-booking::') || rawId.startsWith('sg/'))
