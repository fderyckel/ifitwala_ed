...
-->
	<Dialog v-model="isOpen" :options="{ size: 'xl', title: null }" class="heatmap-dialog">
		<template #body-content>
			<div class="heatmap-surface flex h-[90vh] flex-col gap-4 p-5 text-ink">
				<header class="flex flex-wrap items-center gap-4 border-b border-border/70 pb-3">
					<div class="min-w-0">
						<p class="text-sm uppercase tracking-[0.08em] text-ink/70">Attendance signal view</p>
						<h2 class="text-2xl font-semibold text-ink">Student heatmap</h2>
					...
					<div class="ml-auto flex flex-wrap items-center gap-2">
						<div
							class="flex rounded-full border border-border bg-white/80 p-1 shadow-sm backdrop-blur"
						>
							<button
						...
						<Button appearance="minimal" icon="x" @click="isOpen = false" class="text-ink/60" />
					</div>
				</header>

				<section
					class="flex flex-col gap-3 rounded-xl border border-border/80 bg-white/90 p-4 shadow-sm backdrop-blur"
				>
					<div
						class="grid w-full grid-cols-1 gap-3 md:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_minmax(0,1fr)_auto]"
					>
						<FormControl
							type="select"
							size="md"
						...
						<div class="flex flex-col gap-2">
							...
							<div class="flex flex-wrap gap-2">
								<button
									...
									:class="
										filters.codes.includes(code.value || code.code)
											? 'border-ink/60 bg-ink text-white shadow-sm'
											: 'text-ink/75'
									"
								>
									<span>{{ code.label || code.value || code.code }}</span>
									<span v-if="code.description" class="text-[10px] uppercase tracking-wide">
										{{ code.description }}
									</span>
								</button>
				...
				<section
					class="grid flex-1 grid-cols-1 gap-4 lg:grid-cols-[minmax(0,3fr)_minmax(280px,1fr)]"
				>
					<div
						class="relative flex h-full flex-col overflow-hidden rounded-xl border border-border/80 bg-white/90 shadow-md backdrop-blur"
					>
						<div class="flex items-center justify-between border-b border-border/70 px-4 py-3">
							...
							<div class="flex items-center gap-2">
								...
								<Badge
									v-if="
										filters.course || filters.instructor || filters.term || filters.rotationDay
									"
									variant="subtle"
									class="text-xs font-semibold text-ink"
								>
						...
						<div class="relative flex flex-1 flex-col">
							...
							<div
								v-if="!hasData"
								class="flex flex-1 flex-col items-center justify-center gap-3 p-8 text-center text-ink/70"
							>
								<FeatherIcon name="grid" class="h-10 w-10 text-ink/30" />
								<p class="text-base font-semibold">No heatmap data to display</p>
								<p class="max-w-md text-sm">
									Adjust filters or load attendance data for this student and academic year to see
									signal patterns.
								</p>
							</div>
					...
					<aside
						class="flex h-full flex-col gap-4 rounded-xl border border-border/80 bg-white/95 p-4 shadow-md backdrop-blur"
					>
						<div>
							<p class="text-xs font-semibold uppercase tracking-wide text-ink/70">
								Status legend
						...
						<div class="rounded-lg border border-border/80 bg-slate-50/60 p-3">
							...
							<ul class="mt-2 space-y-1 text-sm leading-relaxed text-ink/80">
								<li v-if="mode === 'whole-day'">
									Whole-day squares aggregate all attendance codes per calendar date. Greyed cells
									are weekends/holidays from the school calendar; dark cells are missing records on
									school days.
								</li>
								<li v-else>
									Per-block squares track the student’s scheduled courses. Light grey indicates no
									class in that block; dark cells flag missing attendance in a scheduled class.
								</li>
								<li>Tooltip shows the date, status, and recorded course-level detail.</li>
								<li>
									Use attendance code filters to spotlight unexcused patterns or late streaks
									quickly.
								</li>
							</ul>
						</div>
...
const visualPieces = computed(() => {
	...
	const dataSource =
		mode.value === 'whole-day' ? normalizedWholeDayPoints.value : normalizedBlockPoints.value;
...
const legendEntries = computed(() =>
	severityScale.value.slice().sort((a, b) => a.score - b.score)
);

const hasData = computed(() =>
	mode.value === 'whole-day'
		? !!normalizedWholeDayPoints.value.length
		: !!normalizedBlockPoints.value.length
);
...
const filteredWholeDayPoints = computed(() => {
	return (props.wholeDayPoints || []).filter(point => {
		...
		if (
			...
			point.academic_year &&
			point.academic_year !== filters.academicYear
		)
			return false;
		if (filters.codes.length && point.status_code && !filters.codes.includes(point.status_code))
			return false;
		...
		if (
			filters.instructor &&
			point.instructor &&
			!point.instructor.toLowerCase().includes(filters.instructor.toLowerCase())
		) {
			return false;
		}
		return true;
...
const filteredBlockPoints = computed(() => {
	return (props.blockPoints || []).filter(point => {
		...
		if (
			filters.course &&
			point.course &&
			!point.course.toLowerCase().includes(filters.course.toLowerCase())
		) {
			return false;
		}
		if (
...
onMounted(() => {
	...
	baseSeverityScale.value = [
		...
		{
			...baseSeverityScale.value[3],
			color: mixColors(palette.value.flame, palette.value.sand, 0.55),
		},
		{
			...baseSeverityScale.value[4],
			color: mixColors(palette.value.clay, palette.value.sand, 0.6),
		},
...
function buildBlockOption(): ChartOption {
	const data = normalizedBlockPoints.value.map(point => {
		const axisX =
			blockXAxisLookup.value.get(`${point.week_index || 0}-${point.weekday_index || 0}`) ?? 0;
		...
		return {
			...
			block_label:
				point.block_label ||
				props.blockLabels[point.block_number] ||
				`Block ${point.block_number}`,
...
function formatWholeDayTooltip(data: any) {
	...
	if (Number.isFinite(data.blocks_scheduled)) {
		lines.push(
			`Blocks: ${data.blocks_scheduled} scheduled${Number.isFinite(data.blocks_absent) ? ` / ${data.blocks_absent} absent` : ''}`
		);
	}
...
function mixColors(hexA: string, hexB: string, weight = 0.5) {
	...
	const mixChannel = (start: number, end: number) => Math.round(start + (end - start) * weight);
