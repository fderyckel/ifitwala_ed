	...
	<Dialog v-model="show" :options="{ size: 'xl', title: null }">
		<template #body-content>
			<div class="history-dialog flex h-[640px] max-h-[85vh] flex-col gap-4">
				<!-- HEADER -->
				<header
					class="relative flex items-start justify-between gap-3 overflow-hidden rounded-2xl border border-border/80 bg-gradient-to-br from-white via-[rgb(var(--surface-rgb)/0.96)] to-white px-5 py-4 shadow-soft"
				>
					<div
						class="absolute -left-14 -top-10 h-36 w-36 rounded-full bg-jacaranda/10 blur-3xl"
					></div>
					...
					<div class="relative flex items-center gap-3">
						<div
							class="flex items-center gap-1 rounded-full border border-border/85 bg-white/90 px-1 py-1 shadow-inner backdrop-blur"
						>
							<button
				...
				<!-- MAIN CONTENT -->
				<section
					class="grid flex-1 grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1.65fr)_minmax(0,1fr)]"
				>
					<!-- Chart panel -->
					<div
						class="relative flex flex-col overflow-hidden rounded-2xl border border-border/80 bg-white/95 shadow-strong"
					>
						<div class="flex items-center justify-between border-b border-border/70 px-5 py-3">
							<div class="space-y-0.5">
								...
								<p class="text-sm text-slate-token/80">
									{{
										hasData
											? 'Smooth curve with soft fill to catch shifts quickly.'
											: 'Waiting for data to render.'
									}}
								</p>
							</div>
							<span
								class="inline-flex items-center gap-1 rounded-full bg-surface-soft px-3 py-1 text-[11px] font-semibold text-slate-token/80"
							>
						...
						<div class="relative flex-1">
							<div
								v-if="loading"
								class="absolute inset-0 z-10 flex items-center justify-center bg-white/75 backdrop-blur-sm"
							>
						...
						<div
							class="flex flex-wrap items-center justify-between gap-3 border-t border-border/70 bg-surface-soft/60 px-5 py-3 text-xs text-slate-token/70"
						>
					...
					<div
						class="flex h-full flex-col overflow-hidden rounded-2xl border border-border/80 bg-white shadow-soft"
					>
						<div
							class="flex items-start justify-between gap-3 border-b border-border/70 bg-slate-50/60 px-4 py-3"
						>
							<div class="space-y-1">
								<p
									class="text-[0.7rem] font-semibold uppercase tracking-[0.12em] text-slate-token/70"
								>
									Context & comments
								</p>
								<p class="text-sm text-slate-token/80">
							...
							<div class="flex items-center gap-2">
								<span
									class="inline-flex items-center gap-1 rounded-full border border-border/70 bg-white px-3 py-1 text-[11px] font-semibold text-slate-token/80"
								>
									{{ selectedRangeLabel }}
								</span>
								<span
						...
						<div class="flex-1 space-y-4 overflow-y-auto bg-white/90 p-4">
							<div
								class="rounded-xl border border-border/70 bg-[rgb(var(--surface-rgb)/0.95)] px-3 py-3 text-sm text-slate-token/80 shadow-inner"
							>
								Use this panel to narrate what changed, pin action items, and keep the thread
								anchored to the selected slice.
							</div>
							...
							<div class="grid grid-cols-2 gap-3">
								<div class="rounded-xl border border-border/65 bg-white/90 px-3 py-2 shadow-inner">
									...
									<p class="text-xs text-slate-token/70">Sum across {{ selectedRangeLabel }}</p>
								</div>
								<div class="rounded-xl border border-border/65 bg-white/90 px-3 py-2 shadow-inner">
									<p class="text-[11px] font-semibold uppercase tracking-wide text-slate-token/70">
										Average
									...
									<p class="text-xs text-slate-token/70">Per data point</p>
								</div>
							</div>

							<div class="space-y-2">
								<div class="flex items-center justify-between gap-2">
									<h4 class="text-sm font-semibold text-ink">Comments</h4>
									<span
										class="inline-flex items-center gap-1 rounded-full bg-surface-soft px-3 py-1 text-[11px] font-semibold text-slate-token/80"
									>
								...
								<div
									class="min-h-[180px] rounded-xl border border-border/70 bg-surface-soft/70 p-3 shadow-inner"
								>
									<div
									...
									<div
										v-else
										class="flex max-h-64 flex-col gap-3 overflow-y-auto custom-scrollbar pr-1"
									>
										<article
						...
						<div
							class="sticky bottom-0 border-t border-border/70 bg-white/95 px-4 py-3 shadow-inner"
						>
							<form
