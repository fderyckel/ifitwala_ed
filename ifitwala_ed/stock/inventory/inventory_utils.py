...
def resolve_issued_to(doc):
	...
	if not value:
		frappe.throw(_("{0} is required for Issued To Type {1}.").format(
			frappe.bold(unscrub(fieldname)), issued_to_type
		))
...
def resolve_returned_from(doc):
	...
	value = doc.get(fieldname)
	if not value:
		frappe.throw(_("{0} is required for Returned From Type {1}.").format(
			frappe.bold(unscrub(fieldname)), returned_from_type
		))
...
def assert_unit_held_by(unit_name, custodian_type, custodian_name):
	...
	if current != custodian_name:
		frappe.throw(_("Inventory Unit {0} is not held by {1} {2}.").format(
			unit_name, custodian_type, custodian_name
		))
...
def _throw_time_coercion(value, fieldname):
	...
	frappe.throw(
		_("Unable to coerce datetime for {0}. Debug: {1}.").format(
			fieldname, frappe.as_json(debug)
		),
