...
class Location(Document):
	...
	def _inherit_org_from_parent(self):
		...
		if not parent_org:
			...
			frappe.msgprint(
				_(
					"Parent Location {0} has no Organization set; "
					"please fix the parent first."
				).format(
	...
	def _validate_org_against_parent(self):
		...
		if self.organization and self.organization != parent_org:
			frappe.throw(
				_(
					"Child Location Organization must match its parent. "
					"Parent: <b>{0}</b>, Child: <b>{1}</b>."
				).format(parent_org, self.organization),
	...
	def validate_capacity_against_groups(self):
		...
		# 3) Detect violations
		over_capacity = [
			(r["name"], cint(r["active_count"]))
			for r in count_rows
			if cint(r["active_count"]) > cap
		]

		if over_capacity:
			lines = "\n".join(
				f"- {sg}: active students {count} > capacity {cap}"
				for sg, count in over_capacity
			)
			frappe.throw(
				_(
					"Cannot set maximum capacity below active enrollment "
					"for Student Groups using this Location:\n{0}"
				).format(lines),
	...
	def _validate_school_lineage_against_parent(self):
		...
		if parent_school not in allowed:
			frappe.throw(
				_(
					"Parent Location belongs to School {0}, "
					"which is not in the lineage of School {1}."
				).format(parent_school, self.school),
	...
	def _validate_location_type_capabilities(self):
		...
		if lt.is_container and not self.is_group:
			frappe.msgprint(
				_(
					"Location Type implies this Location must be a group. "
					"'Is Group' has been enabled."
				),
	...
	def _warn_on_stock_location_without_school(self):
		...
		should_warn = (
			self.is_new()
			or self._has_value_changed_safe("location_type")
			or self._has_value_changed_safe("school")
		)
