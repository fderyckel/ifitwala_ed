...
def get_program_chart_data(data, filters=None):
    ...
    if school_filter and program_filter:
        ...
        return {
            "data": {"labels": labels, "datasets": [{"name": program_filter, "values": values}]},
            "type": "bar",
            "colors": ["#7cd6fd"],
    ...
    if school_filter and not program_filter:
        ...
        for program, year_map in program_map.items():
            ...
            datasets.append({"name": program, "values": values, "chartType": "bar"})

        return {
            "data": {"labels": sorted_years, "datasets": datasets},
            "type": "bar",
    ...
    if not school_filter:
        ...
        labels = sorted(totals.keys(), key=lambda label: (year_start[label], label_school[label]))

        # 3) colour map – first N from palette, then md5‑hash fallback
        base_palette = ["#7cd6fd", "#00b894", "#ff9f43", "#5e64ff", "#ff5858", "#00b0f0", "#ffa3ef"]
        ...
        for lbl in labels:
            ...
            if abbr not in school_colour:
                school_colour[abbr] = (
                    base_palette[palette_idx] if palette_idx < len(base_palette) else _hash_color(abbr)
                )
                palette_idx += 1
...
def get_cohort_data(filters):
    ...
    params = {"student_cohort": filters["student_cohort"], "allowed_schools": allowed_schools}
...
def get_course_chart_data(data):
    ...
    status_colors = {"Enrolled": "#7cd6fd", "Completed": "#5e64ff", "Dropped": "#ff5858"}
