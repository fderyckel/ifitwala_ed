...
from ifitwala_ed.schedule.api.calendar.export import (
    PDF_OPTIONS,
...
class TestCalendarExport(TestCase):
    ...
    def test_resolve_brand_context_falls_back_to_parent_school_logo_and_tagline(self):
        with (
            patch(
                "ifitwala_ed.schedule.api.calendar.export.get_ancestor_schools",
                return_value=["SCHOOL-CHILD", "SCHOOL-PARENT"],
            ),
            patch("ifitwala_ed.schedule.api.calendar.export.get_ancestor_organizations", return_value=["ORG-1"]),
            patch("ifitwala_ed.schedule.api.calendar.export.frappe.db.get_value") as mock_get_value,
            patch("ifitwala_ed.schedule.api.calendar.export.frappe.get_all") as mock_get_all,
        ):
    ...
    def test_export_staff_timetable_pdf_sets_inline_pdf_response(self):
        with (
            patch("ifitwala_ed.schedule.api.calendar.export._system_tzinfo", return_value=pytz.timezone("UTC")),
            patch("ifitwala_ed.schedule.api.calendar.export.now_datetime", return_value=datetime(2026, 4, 20, 9, 0, 0)),
            patch(
                "ifitwala_ed.schedule.api.calendar.export._resolve_employee_for_user",
                return_value={
            ...
            patch(
                "ifitwala_ed.schedule.api.calendar.export.calendar_staff_feed.get_staff_calendar",
                return_value={"events": [], "counts": {}},
            ),
            patch(
                "ifitwala_ed.schedule.api.calendar.export._build_staff_timetable_context",
                return_value={"weeks": [], "count_cards": []},
            ),
            patch(
                "ifitwala_ed.schedule.api.calendar.export._render_staff_timetable_export_html", return_value="<html />"
            ),
            patch("ifitwala_ed.schedule.api.calendar.export._render_staff_timetable_pdf", return_value=b"%PDF-test"),
        ):
