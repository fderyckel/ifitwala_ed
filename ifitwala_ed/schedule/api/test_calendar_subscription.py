...
class TestCalendarSubscription(TestCase):
    def test_staff_subscription_ics_filters_weekly_off_and_escapes_event_text(self):
        ...
        with (
            patch("ifitwala_ed.schedule.api.calendar.subscription._system_tzinfo", return_value=tzinfo),
            patch(
                "ifitwala_ed.schedule.api.calendar.subscription.now_datetime",
                return_value=datetime(2026, 4, 28, 8, 0, 0),
            ),
            patch(
                "ifitwala_ed.schedule.api.calendar.subscription.frappe.utils.get_url",
                return_value="https://school.example",
            ),
        ):
    ...
    def test_download_staff_calendar_subscription_sets_ics_response_without_fetch_writes(self):
        ...
        with (
            patch(
                "ifitwala_ed.schedule.api.calendar.subscription._resolve_subscription_token",
                return_value={"name": "SUB-1", "user": "staff@example.com"},
            ),
            patch(
                "ifitwala_ed.schedule.api.calendar.subscription._subscription_window",
                return_value=(window_start, window_end),
            ),
            patch(
                "ifitwala_ed.schedule.api.calendar.subscription.calendar_staff_feed.get_staff_calendar_for_user",
                return_value={"events": []},
            ) as get_feed,
            patch(
                "ifitwala_ed.schedule.api.calendar.subscription.build_staff_calendar_ics",
