# ifitwala_ed/schedule/api/test_portal_calendar.py
...
from ifitwala_ed.schedule.api.calendar.core import _resolve_employee_for_user, _system_tzinfo
from ifitwala_ed.schedule.api.calendar.staff_feed import (
...
class TestPortalCalendar(FrappeTestCase):
    ...
    def test_booking_backed_class_events_include_location_meta(self):
        ...
        with (
            patch("ifitwala_ed.schedule.api.calendar.staff_feed.frappe.db.table_exists", return_value=True),
            patch("ifitwala_ed.schedule.api.calendar.staff_feed.frappe.db.sql", return_value=[booking_row]),
            patch("ifitwala_ed.schedule.api.calendar.staff_feed._course_meta_map", return_value={}),
        ):
    ...
    def test_staff_calendar_school_events_ignore_guardian_only_audiences(self):
        ...
        with (
            ...
            patch(
                "ifitwala_ed.schedule.api.calendar.staff_feed.get_ancestor_schools",
                return_value=[self.child_school, self.parent_school],
            ),
            patch("ifitwala_ed.schedule.api.calendar.staff_feed.frappe.db.sql", return_value=[event_row]),
            patch("ifitwala_ed.schedule.api.calendar.staff_feed.frappe.get_all", side_effect=fake_get_all),
            patch("ifitwala_ed.schedule.api.calendar.staff_feed.frappe.get_roles", return_value=["Employee"]),
            patch("ifitwala_ed.schedule.api.calendar.staff_feed.get_user_membership", return_value={"teams": set()}),
        ):
    ...
    def test_applicant_interview_school_events_are_participant_only(self):
        tzinfo = _system_tzinfo()
        window_start = tzinfo.localize(datetime(2026, 1, 7, 8, 0, 0))
        window_end = tzinfo.localize(datetime(2026, 1, 7, 12, 0, 0))
        ...
        participant_row = frappe._dict(dict(event_row, participant_name="Staff User"))
        with (
            patch(
                "ifitwala_ed.schedule.api.calendar.staff_feed._resolve_employee_for_user",
                return_value={"name": "EMP-0001", "school": self.child_school},
            ...
            patch("ifitwala_ed.schedule.api.calendar.staff_feed.frappe.db.sql", return_value=[participant_row]),
            patch("ifitwala_ed.schedule.api.calendar.staff_feed.frappe.get_all", return_value=[]),
            patch("ifitwala_ed.schedule.api.calendar.staff_feed.frappe.get_roles", return_value=["Employee"]),
            patch("ifitwala_ed.schedule.api.calendar.staff_feed.get_user_membership", return_value={"teams": set()}),
        ):
            participant_events = _collect_school_events("staff@example.com", window_start, window_end, tzinfo)

        self.assertEqual(len(participant_events), 1)
        self.assertEqual(participant_events[0].meta.get("reference_type"), "Applicant Interview")
    ...
    def test_staff_calendar_school_events_include_team_audience_via_employee_membership(self):
        ...
        with (
            ...
            patch("ifitwala_ed.schedule.api.calendar.staff_feed.get_user_membership", return_value={"teams": {team}}),
        ):
