...
from ifitwala_ed.schedule.api.calendar import invalidation as calendar_invalidation
...
class TestCalendarInvalidation(TestCase):
    def test_active_employee_names_for_school_scope_includes_descendant_scope(self):
        ...
        with (
            patch(
                "ifitwala_ed.schedule.api.calendar.invalidation.get_descendant_schools",
                return_value=["SCHOOL-1", "SCHOOL-2"],
            ),
            patch("ifitwala_ed.schedule.api.calendar.invalidation.frappe.get_all", side_effect=fake_get_all),
        ):
    ...
    def test_invalidate_staff_calendar_for_users_resolves_employee_cache_keys(self):
        with (
            patch(
                "ifitwala_ed.schedule.api.calendar.invalidation.active_employee_names_for_users",
                return_value=["EMP-1", "EMP-2"],
            ) as resolver,
            patch(
                "ifitwala_ed.schedule.api.calendar.invalidation.invalidate_staff_portal_calendar_cache"
            ) as invalidator,
        ):
