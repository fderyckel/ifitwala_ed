...
FAST_TRACK_TITLE = _("Fast-Track Enrollment Finished")
...
def _run_fast_track_job(request_names: list[str], enrollment_date: str, target_user: str):
    previous_user = frappe.session.user
    frappe.set_user(target_user)
    try:
        _assert_fast_track_permissions(user=target_user)
        _execute_fast_track(
...
def get_fast_track_access():
    ...
    return {
        "can_run": 1 if can_run else 0,
        "reason": reason,
    }


@frappe.whitelist()
def preview_fast_track_requests(filters=None):
    _assert_fast_track_permissions(user=frappe.session.user)
    ...
    counts = Counter(selected=len(requests))
    ...
    return {
        "counts": dict(counts),
        "default_enrollment_date": str(_resolve_default_enrollment_date(filters.get("academic_year"))),
...
@frappe.whitelist()
def run_fast_track_requests(filters=None, enrollment_date=None):
    _assert_fast_track_permissions(user=frappe.session.user)
    filters = _normalize_fast_track_filters(filters)
    _normalized, requests = get_filtered_requests(filters, require_matrix_offering=False)
    request_names = [request.get("name") for request in requests if request.get("name")]
    ...
    resolved_enrollment_date = _resolve_requested_enrollment_date(
    ...
    if len(request_names) > FAST_TRACK_QUEUE_THRESHOLD:
        frappe.enqueue(
            _run_fast_track_job,
            queue="long",
            job_name=f"Fast-track {len(request_names)} Program Enrollment Requests",
            request_names=request_names,
            enrollment_date=str(resolved_enrollment_date),
            target_user=target_user,
        )
        return {
            "queued": 1,
...
def _get_fast_track_access(*, user: str | None):
    ...
    if not frappe.has_permission("Program Enrollment Request", ptype="write", user=actor):
        return False, PERMISSION_ERROR_REQUEST
    ...
    if not can_create_enrollment or not can_write_enrollment:
        return False, PERMISSION_ERROR_ENROLLMENT

    return True, ""


def _assert_fast_track_permissions(*, user: str | None):
...
def _publish_progress(*, position: int, total: int, target_user: str):
    frappe.publish_realtime(
        FAST_TRACK_PROGRESS_EVENT,
        {
            "action_label": _("Approving and creating enrollments"),
...
def _finalize(*, counts: dict, issues: list[tuple[str, str, str]], target_user: str, batch_mode: bool):
    summary = {
        "title": FAST_TRACK_TITLE,
        "counts": counts,
    }
...
def _execute_fast_track(*, request_names: list[str], enrollment_date: str, target_user: str, batch_mode: bool):
    names = [(name or "").strip() for name in (request_names or []) if (name or "").strip()]
    if not names:
    ...
    for position, request_name in enumerate(names, start=1):
        ...
        try:
            ...
            request_doc.add_comment(
                "Comment",
                _("Fast-track approved and materialized on {0}.").format(enrollment_date),
            )
            ...
            _publish_progress(position=position, total=total, target_user=target_user)
