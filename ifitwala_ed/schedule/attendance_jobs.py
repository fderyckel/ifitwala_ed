...
import frappe
from frappe.utils import add_to_date, get_datetime, now_datetime, nowdate, time_diff_in_seconds

from ifitwala_ed.schedule.attendance_utils import LIMIT_DEFAULT, SG_SCHEDULE_DT, get_meeting_dates
...
def _rotation_days_by_group(names: List[str]) -> Dict[str, Set[int]]:
    ...
    if not names:
        return {}
    rows = frappe.db.get_all(SG_SCHEDULE_DT, filters={"parent": ["in", names]}, fields=["parent", "rotation_day"])
...
def prewarm_meeting_dates(limit: int | None = None) -> dict:
    ...
    names = [g["name"] for g in groups if rot_by_pair.get((g["school_schedule"], g["academic_year"]))]
    if not names:
        return {"warmed": 0, "candidates": 0}
    ...
    candidates = [
        g["name"]
        for g in groups
        if (r := rot_by_pair.get((g["school_schedule"], g["academic_year"]))) and r in rd_by_group.get(g["name"], set())
    ]
