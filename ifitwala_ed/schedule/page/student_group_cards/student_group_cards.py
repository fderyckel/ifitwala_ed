...
def get_context(context):
    ...
    context.cohorts = frappe.get_all("Student Cohort", fields=["name", "cohort_name"])
...
def get_student_group_students(student_group, start=0, page_length=25):
    student_data = frappe.get_all(
        ...
        as_list=True, start=start, 
        page_length=page_length
    )
    
    student_ids = [s[0] for s in student_data]
    student_details = frappe.db.get_values("Student", {"name": ("in", student_ids)}, ["name", "student_preferred_name", "image"], as_dict=True)
    ...
    for student_id, student_name in student_data:
        ...
        students.append({
            ...
            "image": student_info.get("image", "")
        })
        student["preferred_name"] = student_doc.student_preferred_name if hasattr(student_doc, "student_preferred_name") else ""
        student["image"] = student_doc.image if hasattr(student_doc, "image") and student_doc.image else ""
...
def reset_student_fetch(student_group):
    return {"students": get_student_group_students(student_group, start=0, page_length=25), "start": 25, "total": frappe.db.count("Student Group Student", {"parent": student_group})}
