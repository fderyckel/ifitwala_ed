...
def _coerce_filters(raw):
    ...
    if isinstance(raw, list):
        return {f["fieldname"]: f.get("value") or f.get("default") for f in raw if f.get("value") or f.get("default")}
...
def get_instructor_events(start, end, filters=None):
    ...
    groups = (
        frappe.qb.from_(SG)
        .inner_join(SGSchedule)
        .on(SGSchedule.parent == SG.name)
        .select(
            SG.name,
    ...
    for grp in groups:
        ...
        rotation_dates = get_rotation_dates(sched_name, eff_year, sched_doc.include_holidays_in_rotation)
        ...
        slots = (
            ...
            .select(SGSchedule.rotation_day, SGSchedule.block_number, SGSchedule.location, SGSchedule.instructor)
        ...
        for sl in slots:
            ...
            if block_meta is None:
                block_meta = frappe.db.get_value(
                    ...
                    {"parent": sched_name, "rotation_day": sl.rotation_day, "block_number": sl.block_number},
            ...
            color = (
                get_course_block_colour(school)
                if block_meta.block_type == "Course"
                else get_block_colour(block_meta.block_type)
            )
