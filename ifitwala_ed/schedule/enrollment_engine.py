        ...
        if rule_type == "MIN_TOTAL_COURSES":
            ...
            if not group:
                ...
                if int(group_summary.get(group, 0) or 0) <= 0:
                    _violate(
                        "require_group_coverage", f"Basket must include at least one course from group '{group}'.", rule
                    )
            ...
            _violate(
                "unsupported_rule",
                "Basket rule MIN_LEVEL_COUNT is not supported yet (level mapping not provided to engine).",
                rule,
            )
