...
class StudentGroup(Document):
    ...
    def before_save(self):
        ...
        def rotation_days_set(doc):
            return {int(r.rotation_day) for r in (doc.student_group_schedule or []) if getattr(r, "rotation_day", None)}
    ...
    def validate_students(self):
        ...
        enabled_records = frappe.db.get_values(
            "Student", {"name": ["in", student_names]}, ["name", "enabled"], as_dict=True
        )
    ...
    def validate_rotation_clashes(self):
        ...
        for row in self.student_group_schedule or []:
            ...
            for instr in self.instructors or []:
                identifier = (getattr(instr, "employee", "") or "").strip() or (
                    getattr(instr, "instructor", "") or ""
                ).strip()
                if not identifier:
                    continue
    ...
    def _validate_overlap_conflicts(self):
        ...
        def _group_links(payload):
            ...
            links = [get_link_to_form("Student Group", sg) for sg in sgs if sg]
        ...
        if ins_conf:
            for payload in ins_conf:
                ...
                messages.append(
                    _("Instructor(s) {ins} already booked on rotation day {rot}, block {blk} in {sg}.").format(
                        ins=ins_list, rot=rot, blk=blk, sg=sg_links
                    )
                )
        ...
        if stu_conf:
            for payload in stu_conf:
                ...
                sg_links = _group_links(payload)
                messages.append(
                    _("Student(s) {stu} already booked on rotation day {rot}, block {blk} in {sg}.").format(
                        stu=stu_list, rot=rot, blk=blk, sg=sg_links
                    )
                )
    ...
    def _get_school_schedule(self):
        ...
        if self.school_schedule:
            ...
            base_school = (
                frappe.db.get_value("Program Offering", self.program_offering, "school")
                if self.program_offering
    ...
    def _enforce_school_rules(self):
        ...
        po_school = (
            frappe.db.get_value("Program Offering", self.program_offering, "school") if self.program_offering else None
        )
...
def get_program_enrollment_offering_first(
