...
class EndofYearChecklist(Document):
	...
	def archive_academic_year(self):
		...
		frappe.qb.update(AY).set(AY.archived, 1).set(AY.visible_to_admission, 0).where(
			AY.name.isin(ay_names)
		).run()
	...
	def archive_terms(self):
		...
		Term = DocType("Term")
		frappe.qb.update(Term).set(Term.archived, 1).where(
			Term.academic_year.isin(ay_names)
		).run()
		return {"processed": len(ay_names)}
	...
	def archive_program_enrollment(self):
		...
		frappe.qb.update(ProgramEnrollment).set(ProgramEnrollment.archived, 1).where(
			(ProgramEnrollment.academic_year.isin(ay_names))
			& (ProgramEnrollment.school.isin(schools))
		).run()
		return {"processed": len(ay_names)}
	...
	def archive_student_groups(self):
		...
		frappe.qb.update(StudentGroup).set(StudentGroup.status, "Retired").where(
			(StudentGroup.academic_year.isin(ay_names))
			& (StudentGroup.school.isin(schools))
		).run()
		return {"processed": len(ay_names)}
