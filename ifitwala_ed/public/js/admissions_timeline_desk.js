...
(function () {
	...
	const SEND_CASE_MESSAGE_METHOD = "ifitwala_ed.api.admissions_communication.send_admissions_case_message";
	...
	function getContextArgs(timeline, fallback) {
		...
		return {
			...
			student_applicant: clean(context.student_applicant || (doctype === "Student Applicant" ? name : "")),
	...
	function actionButtonHtml(action, index) {
		...
		const reason = !enabled && action?.disabled_reason
	...
	function actionsHtml(timeline) {
		const actions = (timeline?.actions || []).filter(action => clean(action.id) !== "open_timeline");
		if (!actions.length) {
			return "";
		}
		return `
	...
	function openTimelineDialog(options) {
		...
		dialog.show();
		dialog.get_field("timeline_html").$wrapper.html(`<div class="text-muted">${__("Loading admissions timeline...")}</div>`);
		refreshTimelineDialog(dialog, state);
	}
	...
	function loadContextThenRun(options, actionId) {
		...
		loadTimeline(contextDoctype, contextName, 50)
			.then((timeline) => {
				...
				if (!action) {
					frappe.msgprint(__("This contextual action is not available for the current admissions record."));
					return;
				}
	...
	function promptLogActivity(state) {
		...
		if (!context.conversation) {
			frappe.msgprint(__("Log a message first so an admissions conversation exists for this activity."));
			return;
		}
		const dialog = new frappe.ui.Dialog({
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				{ fieldname: "visit_column_break", fieldtype: "Column Break" },
				{
					...
					fieldtype: "Select",
					options: (payload?.visit_types || ["Family Tour", "Student Tour", "Open Day", "School Visit", "Other"]).join("\n"),
			...
			primary_action(values) {
				...
				frappe.call({
					...
					if (result.ok) {
						frappe.show_alert({ message: __("Admissions visit scheduled."), indicator: "green" });
						dialog.hide();
						state.sourceFrm?.reload_doc?.();
						return;
					}
	...
	function renderVisitSuggestions(dialog, suggestions) {
		...
		if (!Array.isArray(suggestions) || !suggestions.length) {
			field.$wrapper.html(`<div class="text-muted small">${__("No suggested free times yet.")}</div>`);
			return;
		}
	...
	function promoteApplicant(action, state) {
		const target = clean(action?.target) || getContextArgs(state.timeline, state).student_applicant;
		...
		if (target) {
			...
			frappe.show_alert({ message: __("Open the applicant form and use Promote when ready."), indicator: "blue" });
		}
	}
	...
	function inviteToApply(action, state) {
		...
		const frm = state.sourceFrm;
		if (frm?.doctype === "Inquiry" && clean(frm.doc?.name) === target && typeof frm.trigger === "function") {
			frm.trigger("convert_to_applicant");
			return;
		}
		if (target) {
			frappe.set_route("Form", "Inquiry", target);
			frappe.show_alert({ message: __("Open the Inquiry form and use Invite to Apply."), indicator: "blue" });
		}
	}
	...
	function archiveContext(action, state) {
		...
		if (context.conversation) {
			...
			frappe.show_alert({ message: __("Open the admissions conversation and archive it there."), indicator: "blue" });
			return;
		}
		if (target) {
			frappe.set_route("Form", "Inquiry", target);
